...
function buildGeoBlock(){
  ...
  if(_stashKind&&typeof villageStash==="function"){
    var _stRow=function(r){return r.name+(r.qty>1?" ×"+r.qty:"")+(r.room?" — "+r.room:"")+(r.by?" (left t"+r.placed+" by "+r.by+")":"");};/* #408 ④: the room rides the row, the name is untouched */
  ...
  /* #408 ⑤: the ROOM GRAPH is served every turn while inside a place that has one (~80 tokens for six rooms) — canon the GM
     cannot narrate around: no cellar the record lacks, no door it never listed. Placed items ride with their room. */
  if(activeNode&&activeNode.layout&&activeNode.layout.rooms&&activeNode.layout.rooms.length){
    var _lyRooms=activeNode.layout.rooms.map(function(r){return r.name+" ("+r.size+")"+(r.features?": "+r.features:"")+" — opens onto "+r.to.join(", ");});
    var _lyPlaced=(activeNode.items||[]).filter(function(it){return it.room&&!it.taken&&(it.qty===undefined||it.qty>0);}).map(function(it){return "PLACED: "+it.name+(it.qty>1?" ×"+it.qty:"")+" — "+it.room;});
    lines.push("LAYOUT (canon — the rooms of this place and how they connect; do not invent a room or a door not listed; to set an item down in a room: [LOCATION_ITEM:name|placed|"+locDisplayLeaf(_activeKey)+"|<room>, <spot>]): "+_lyRooms.join(". ")+(_lyPlaced.length?". "+_lyPlaced.join(". "):""));
  }
  ...
  if(typeof kindDef==="function"&&kindDef().hall&&typeof villageHallKey==="function"&&_activeKey===locResolve(villageHallKey())){var _hn=memory.map.nodes[_activeKey]||{},_hm=_hn.mementos||[],_hw=_hn.wall||[];
    if(_hm.length)lines.push("THE HALL (mementos of finished campaigns \u2014 the objects are decoration; every memento names one thing its tale never resolved): "+_hm.map(function(m){return m.resident+" \u2014 "+m.object+": "+m.fate+" Unresolved: "+m.unresolved+(m.line?" Their own line: \""+m.line+"\"":"");}).join(" | "));
    if(_hw.length)lines.push("THE WALL OF THE UNFINISHED (named, not resolved): "+_hw.map(function(w){return w.resident+" ("+w.campaign+")";}).join("; "));}
  // Known sub-locations
...
function buildWhispersNote(){
  ...
  var cm=(worldState.character&&worldState.character.coreMemories)||[],last=cm.length?String(cm[cm.length-1].text||"").slice(0,120):"";/* #450: a 120-char gist — the full memory was being retold verbatim */
  ...
  return "[ENGINE NOTE — WHISPERS (not a player action): "+label+" is a place where people talk. Let ONE of these mention "+((_wpool&&typeof kindDef==="function"&&kindDef().whisperSubject)||"what is said about the party — ")+_wsrc.join(" or ")+" (someone who has been elsewhere; NEVER a companion who has been at the player's side, they have heard nothing the player has not) — hearsay that has passed through several mouths before reaching this one: ONE sentence, in character, and it must NOT read as a report of what happened. Seeds, never to be retold as they stand: "+(dec.length?"decisions — "+dec.join("; ")+". ":"")+(qs.length?"finished quests — "+qs.join("; ")+". ":"")+(last?"defining moment — "+last+" ":"")+(resFacts.length?"what the residents here lived through in their own campaigns — "+resFacts.join("; ")+". ":"")+"Distort on purpose — keep at most one true detail; lose or garble the names, double the numbers, credit the wrong hero, invent a cause; someone who was there should barely recognise the event. It never invents an event that did not happen, only tells one badly. Emit [WHISPER:one sentence of what is said] so the engine remembers the rumour as rumour. If no one here would have heard anything, say nothing and emit nothing."+(said.length?" Already said, in words or substance — do not repeat: \""+said.join("\" / \"")+"\". A new rumour, or nothing.":"")+"]";
}
...
function buildSoundscapeNote(){
  if(!worldState||worldState.combat||!worldState.world||!worldState.world.location)return "";
  var key=locResolve(currentNodeKey()),node=memory.map&&memory.map.nodes[key];if(!node)return "";
  var current=audioCurrentScene();
  if(audioNodeProfile(node)||(!node.soundscape&&current.campaignKind==="village"&&(current.exterior||current.common)))return "";
  var visit=String(node.visits||0)+"/"+audioNodeStamp(node),ask=worldState.audioAsk;
  if(ask&&ask.node===key&&ask.visit===visit)return "";
  worldState.audioAsk={node:key,visit:visit};
  return "[ENGINE NOTE - SOUNDSCAPE (not a player action): classify the place where THIS response actually ends, using only its established player-perceivable surroundings. Emit [SOUNDSCAPE:explicit final place name|enclosure=open;setting=wilderness;biome=temperate;quiet=normal;allows=birds,insects,wind;forbid=none]. Values: enclosure open/covered/sealed/unspecified; setting settlement/wilderness/interior/subterranean/coast/unspecified; biome temperate/arid/tropical/frozen/unspecified; quiet normal/hushed/silent. allows/forbid are comma lists of "+AUDIO_CONTENTS.join(",")+", or none. Allow only sounds supported by the public scene, never hidden actors, future events or generic genre expectations. Use unspecified/none when unsure. Do not describe a sound just to satisfy this request. This is cosmetic metadata, never new canon; do not mention this note.]";
}
// #408 ② (owner rulings 2026-09-14): the LAYOUT ask — the #207 ③ hours-ask pattern. Fires ONCE at a village HOUSE
// (a node with an owner) whose record holds no layout; anywhere else only when armLayoutAsk(key) armed it (Table
// Talk's "no floor plan is recorded" offers that click). The note is the ONLY place the GM learns the LAYOUT syntax
// (engine-only tier) and the room operand of LOCATION_ITEM. Combat-silent; latch worldState.layoutAsk={node,turn};
// the arm (worldState.layoutAskArmed) is consumed on firing.
function buildLayoutNote(){
  if(!worldState||worldState.combat||typeof memory==="undefined"||!memory||!memory.map||!worldState.world||!worldState.world.location)return"";
  var key=(typeof currentNodeKey==="function")?currentNodeKey():null;if(!key)return"";if(typeof locResolve==="function")key=locResolve(key);
  var node=memory.map.nodes[key];if(!node||node.layout)return"";
  var armed=worldState.layoutAskArmed===key;
  var house=!!(node.owner&&typeof kindDef==="function"&&kindDef().stashQuantities);
  if(!armed&&!house)return"";
  var la=worldState.layoutAsk;if(la&&la.node===key&&!armed)return"";
  if(armed)delete worldState.layoutAskArmed;
  worldState.layoutAsk={node:key,turn:worldState.turn};
  var label=(typeof locDisplayLeaf==="function")?locDisplayLeaf(key):key;
  return "[ENGINE NOTE — LAYOUT (not a player action): "+label+" has no floor plan on record. File its ROOM GRAPH once this response, as it stands in the story so far: [LAYOUT:room|size|features|opens onto; room|size|features|opens onto; ...] — one entry per room: its name, a size word (tiny/small/medium/large), its fixed features in a few words, and the rooms it opens onto, comma-separated, with 'outside' for a door to the open air. No coordinates; every connection must name a listed room or 'outside'; a one-room place is one entry. This becomes canon and is served back every turn: never narrate a room or a door it does not hold. From then on an item set down in a room is tagged [LOCATION_ITEM:name|placed|"+label+"|<room>, <spot>] with the item's own name unchanged. Do not mention this note.]";
}
...
/* #386/#370 ask channel (owner 2026-09-24, after the save scan: zero initiative and zero growth filings across seven
   campaigns — both tags were taught only by the standing doc, marked RARE, and nothing ever asked at a moment; whispers,
   which ride an ask with a cadence, fire everywhere). ONE note, the whisper shape: every COMPANION_INITIATIVE_EVERY turns
   outside combat, when a companion with a flaw is at the player's side and no unbidden act is on the cadence line, it
   names each flaw and asks whether what is in front of the party would move ONE of them — to act unbidden (initiative)
   or, rarer, against the flaw at a defining moment (growth). "Nothing" is a full answer. Latch: worldState.impulseAsk. */
function buildCompanionImpulseNote(){
  if(!worldState||worldState.combat||!(worldState.npcs instanceof Array))return "";
  var every=(typeof COMPANION_INITIATIVE_EVERY==="number")?COMPANION_INITIATIVE_EVERY:12,ia=worldState.impulseAsk;
  if(ia&&typeof ia.turn==="number"&&worldState.turn-ia.turn<every)return "";
  if(typeof companionInitiativeRecent==="function"&&companionInitiativeRecent())return "";/* a recent act already rides the cadence line: not again this soon */
  var flawed=[],i;
  for(i=0;i<worldState.npcs.length;i++){var n=worldState.npcs[i],cs=n&&n.charSheet;
    if(!n||!n.partyMember||n.dead||!cs||!cs.flaw)continue;
    if(cs.splitLoc&&cs.splitLoc.location)continue;/* elsewhere — not at the player's side */
    flawed.push(n.name+": \""+String(cs.flaw).slice(0,140)+"\"");}
  if(!flawed.length)return "";
  worldState.impulseAsk={turn:worldState.turn};
  return "[ENGINE NOTE — COMPANION IMPULSE (not a player action): set what is in front of the party right now against each companion's flaw — "+flawed.join("; ")+". If ONE of them would be moved by their flaw to act unbidden — pocket what glitters, freeze before the priest, blurt the truth — let them, once, this turn: in the open, with a filed [DICE:] roll and a consequence on THEIR sheet, and record it with [COMPANION_INITIATIVE:Name|what they did]. Rarer still: if this is the defining moment where one of them acts AGAINST their flaw, on screen and unasked, record it with [COMPANION_GROWTH:Name|the flaw as written|what replaced it]. Usually neither fits — then do nothing and emit nothing; the flaw is a person, not a tic. At most one companion, never a stranger.]";
}
...
/* #6 C2 (2026-09-13): the RETURN greeting — armed by villageReturnObserve (game.js) after PREVIOUSLY_AFTER_MS of real time,
...
function buildResidentExchangeNote(){
  ...
  var local=(typeof buildSceneManifest==="function")?buildSceneManifest().local:[],res=[],i;
...
var NOTE_SHAPES={
  ...
  buildCompanionImpulseNote:{shape:"cooldown-reminder",latch:["impulseAsk"],combat:"silent",village:"fires",ack:["COMPANION_INITIATIVE","COMPANION_GROWTH"]},/* #386/#370 ask channel */
  ...
  buildSoundscapeNote:{shape:"one-shot-ask",latch:["audioAsk"],combat:"silent",village:"fires",ack:["SOUNDSCAPE"]},
  buildLayoutNote:{shape:"one-shot-ask",latch:["layoutAsk","layoutAskArmed"],combat:"silent",village:"fires",ack:["LAYOUT"]},/* #408 ② */
...
function buildSysPrompt(){
  ...
  var i,nstr="none";if(worldState.npcs.length){var ns=[];for(i=0;i<worldState.npcs.length;i++){var npc=worldState.npcs[i];if(npcIsDead(npc)){_decList.push({n:npc.name,t:(typeof npc.dead==="number"?npc.dead:0)});continue;}if(!_rosterKeep(npc)){_rosterFold++;continue;}var npcAka=npc.aliases&&npc.aliases.length?" [aka: "+npc.aliases.join(", ")+"]":"";/* pronoun fallback: explicit wins; party members derive from charSheet.gender; everyone else defaults to they/them so the GM never has to guess */var npcPr=npc.pronouns||(npc.partyMember&&npc.charSheet&&npc.charSheet.gender?pronounsForGender(npc.charSheet.gender):"they/them");var npcRel=npc.partyMember?relByEntity[npc.name.toLowerCase()]:null;
    ...
    var _moodBit=(npc.status&&npc.statusTurn>0&&npcMoodAge>=0&&npcMoodAge<MOOD_AUDIT_TURNS)?npc.status:"",_moodPushed=false;/* #460 ①: pushed below — first, unless the sheet leads */
...
function diceTxt(t){var out="",di;
  ...
  /* #422: every [DICE:] field is MODEL TEXT that lands in innerHTML (addMsg, the rolled-check swap, transcript replay) —
     escape all three, exactly as the [CHECK:] branch above escapes its label. Unescaped, a hostile provider or a
     tampered transcript could run script in the page (S2, DOC/Research/remote_player_exploration.html §14). */
  var ms=String(t||"").match(/\[DICE:([^\]]+)\]/g);if(!ms)return out;for(di=0;di<ms.length;di++){var mm=ms[di].match(/\[DICE:([^\]]+)\]/);if(!mm)continue;var p=mm[1].split("|");var lbl=p[0]?'<span style="font-size:10px;text-transform:uppercase;letter-spacing:.08em;color:var(--t2);margin-right:8px;">'+escHtml(p[0])+'</span>':'';out+='<div class="dice-block">'+lbl+'d20: <strong>'+escHtml(p[1]||"?")+'</strong>'+(p[2]?" -- "+escHtml(p[2]):"")+'</div>';}return out;}
