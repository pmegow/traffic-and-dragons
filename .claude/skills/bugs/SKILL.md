...
`bug_tracker.html` reads it). Endpoint config: `.claude/bugs.local.json` (gitignored) =
...
1. Read `.claude/bugs.local.json`. Fetch `<endpoint>?s=<secret>` (append `&since=<lastSync>` when
...
2. **Drift-surface flag YES → the drift-protection policy applies in full** (CLAUDE.md ▸ Dev
