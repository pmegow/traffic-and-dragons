...
| `audio-events.js` · `audio-scenes.js` · `ambient.js` · `ui-ambient.js` | ✅ Pilot (L1/L7) | Additive voice/capture events, fixed-bed scene registry, pure ambience policy + bounded controller, and device settings/read-only scene adapter. Shared Sound context; #410 pause/resume intent; same-origin sfx assets. See [pilot brief](DOC/PROPOSAL_Village_Blacksmith_Ambience.md). |
