...
- **⚠ Test sessions must NEVER touch production cloud data (#72, v1.365 — the B7 incident).** Before driving ANY test turns in a browser (harness, multiplayer tests, manual playtests), set `localStorage tnd_test_mode_v1 = "1"` — storage-adapter then refuses every server call loudly (the saved token/URL stay put but never come live). The harness step-1 snippet and install both arm it. A test that genuinely needs a live server (sync features) uses the **locally-run server** — never the production account: the latch deliberately exempts localhost hosts, and the server repo's README ▸ "Local test server" has the whole recipe (`DB_PATH=./test-local.db npm run dev`, then `POST /auth/dev-login` with the secret from its `.env` → `storageAdapter.setServer("http://localhost:4001", sessionId)`; no OAuth popup). Escape hatch for the latch: remove the key + reload.
