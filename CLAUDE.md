...
| `clock.js` | ✅ Active (#73) | The CAMPAIGN CLOCK — ONE monotonic scalar `worldState.clock.min`; the day/hour/minute view is DERIVED, never stored. The GM does ZERO arithmetic: it emits duration estimates (`[TIME_ADVANCE:2h]`), the engine does every add and RECOMPUTES every countdown from the anchor (a number the GM never re-states cannot drift — the invented-solstice-countdown class). Schedule store (`scheduleAdd`/`scheduleDue`) + `buildClockBlock()` prompt injection; per-response advance cap `CLOCK_MAX_RESPONSE_ADVANCE`. Full spec: DOC/DOC_clock.html. **#142 (v1.563): reconcile is skip-and-demand across DAWN** — a `[TIME:]` top-up whose declared phase already passed this engine-day AND >`RECONCILE_SKIP_MIN`(6h) is presumed a mislabel: text kept, roll skipped, muts line + `buildReconcileSkipNudge` demands [REST:long]/[TIME_ADVANCE:Nd]/a corrected [TIME:] (one-shot; the forgotten-rest morning-after heals in one turn). Honest same-day skips and small dawn-approaches reconcile as before |
| `identity.js` | ✅ Active (#156 Phase A) | ⛨ **THE IDENTITY LAYER spine** — `IDENTITY_DOMAINS` registry (npc full adapter; capability/item as already-conforming type domains with `merge:null` — normalization-collapse IS their semantics; location lands Phase B on the A0-ruled additive-ID representation, quest/faction evidence-gated Phase C), `resolveEntity(domain,name)` (npc = `resolveNpcName`, parity-tested; legacy tag call sites deliberately NOT rewired until Phase B needs the shared path), `_identityActionTag` (the `[ALIAS:]`/`[MERGE:]` parse core — pipe/unknown-domain/uncapable refusals all LOUD and mutation-free; npc operands route into the shipping NPC_ALIAS/NPC_MERGE handlers so the vocabularies cannot diverge), **`npcUpsertTarget`** (THE collision boundary in the `[NPC:]` handler: an introduction-shaped write (`NPC_INTRO_REL_RE`) into a history-rich living non-party record lands in a **PROVISIONAL identity** `"Name °tN"` — never blocks, never fuses (Sol §2); cap `PROVISIONAL_CAP`=4 outstanding, beyond it LOUD degrade to the pre-#156 direct write; known honest limit: the t988 false-familiarity class is not engine-detectable — the NAMING clause + suggestion filter attack its base rate), `buildProvisionalNudge` (the same/distinct fork — `[NPC_MERGE:canon\|prov]` folds back, `[MERGE:npc\|Proper Name\|prov]` renames; re-fires per record every `PROVISIONAL_NUDGE_COOLDOWN`=5 turns until resolved, combat-silent without consuming, latch `provisionalNudged` in NOTE_LATCH_FIELDS), `buildNamingClause` (stable-half identity discipline — names ARE keys, endpoint-pair road form, sublocation-not-parent-baked). Every npc merge archives the duplicate's complete pre-image to `memory.archive.identityMerges` (P12; `.tnd` import whitelist carries it); provisional ° keys are excluded from the #128 scan and never become permanent aliases. **Phase B (v1.582) — the location domain**: node keys are **IMMUTABLE NAME-BORN identifiers** + a SPARSE identity overlay `memory.map.identity.entries` ({mergedInto, aliases, display}) — the A0 additive-ID ruling shipped with its criteria intact but zero-migration and old-client-safe (Phase B review call #1: opaque re-keying would corrupt under the stale-SW cross-device window; name-born keys degrade safely). `locResolve` (memoized, generation-bumped, cycle-guarded loud) / `locSame` / `locIsSub` (world-ness = parent relation, NEVER key shape) / `locDisplayLeaf` (fixes the ≥3-segment leaf-display bug). **Executors**: `locMerge` (A0 §7.4 field rules via `locFoldNodeRecords`, tombstone + alias, memory.locations fold, `_locHealLivePointers` — world pointer/combat.node/pendingLocState/locDescNudged/lastArrivalFrom/splitLoc rewritten at repair time — + `_locCompactEdges`; the O(n) historical mass (transcript `e.l`, lastSeenAt) stays as written and RESOLVES at read), `locReparent` (parent pointer, ancestor-cycle guard), `locSplit` (tool-only — human allocation spec, successors carry optional `kind`/`endpoints[]`, fused key tombstones to the PRIMARY so history resolves coarse-but-consistent), `locAliasRegister` (live-key refusal). Resolution seam sites: fileLocation/fileSubLocation/fileLocation* / mapNpcLocation (write-time canonicalization, typeof-guarded), RAG scene + `e.l` scoring via locSame, geo block lookups/sub-listing/connections/NPCs-elsewhere, changed-locations exclusion, scene-manifest presence, affordance travel gate, #133b rejoin, split audit, blueprint export. Repair surface: `dev/loc-repair-core.js` (census — typed evidence, **classifies nothing**; `locRepairApply` plan runner with true dry-run via global-swap clones) + the **map_cleanup.html** satellite. Sabotage: `dev/sabotage-identity.js` (22 cases, Phases A+B). Proposal + plan: [DOC/identity_hardening_fable.html](DOC/identity_hardening_fable.html) §7; A0 ruling: [audits/AUDIT_identity_A0_representation_gate.md](audits/AUDIT_identity_A0_representation_gate.md); Phase A/B records: [audits/AUDIT_identity_phaseA_v1.581.md](audits/AUDIT_identity_phaseA_v1.581.md), [audits/AUDIT_identity_phaseB_v1.582.md](audits/AUDIT_identity_phaseB_v1.582.md) |
...
| `stt.js` | ✅ Active | Speech-to-text input: `STT` module with **two independent paths**. ① **Native** — Web Speech API (`webkitSpeechRecognition`, zero-dependency, no key, no network): Chrome/Edge desktop + Android Chrome; NOT Firefox; iOS Safari has NO SpeechRecognition at all. ② **Cloud fallback** — when native is absent AND an OpenAI key is on file, `MediaRecorder` captures the utterance and OpenAI transcribes it (**#113 §4, v1.536:** `gpt-4o-mini-transcribe` primary with loud `whisper-1` fallback; `sttBiasPrompt()` roster/place/quest vocabulary rides every request; WebAudio silence endpointing — 1.5s quiet ends the take, 45s hard cap; mic `getSettings` telemetry to the #16 crumbs). **This is what makes the iPhone Safari / Car Mode path work** ([history](DOC/CLAUDE_HISTORY.md#sttjs--the-stale-doc-lesson-84)). A fully separate code section below the native one, so native behavior stays byte-identical wherever native exists; `isSupported()` gates the mic button on EITHER path being usable. Both paths run their final transcript through **`sttCorrectNames`** (helpers.js) — roster-phonetic correction for fantasy proper nouns, roster snapshotted once per listen. Prefs: language + auto-send (`tnd_stt_lang_v1`/`tnd_stt_autosend_v1`) and Car Mode auto-listen (`tnd_car_autolisten_v1`, default ON when unset). Notifies the Car Mode overlay via the global `carNotify(kind,text)` (ui-carmode.js), always behind a `typeof` guard. ⚠ **Transcription cost rides the PLAYER's own key** — that ownership property was decisive in declining LiveKit. **#77 confirm gate (v1.548):** Layer-0 confidence captured per utterance (`include[]=logprobs` on gpt-4o transcribe; native `r[0].confidence`; `sttLogEvent` ring `tnd_stt_log_v1`, read via `sttLogAll()`), `sttSuspicion` (helpers.js) gates the AUTO-SEND path only — Car Mode speaks "I heard: … — send it?" with spoken yes/no/redo (`parseConfirmCommand`, same anchored false-positive discipline; the confirm interceptor sits ABOVE carVoiceCommand/busy-park/<3-char gates), non-car auto-send holds + amber-flags the field; File ▸ Admin ▸ 🛡 toggle, default ON; nothing touches sessionLog/transcript until confirmed. Invented tags (the [MANA:-1] leak) are display-stripped by cleanTxt's unknown-tag catch-all (v1.554) |
...
| `item_bible.js` | ✅ Active (#81) | Curated canon for mechanics-bearing carried items — the spell-bible pattern for gear. `ITEM_BIBLE` keyed by `itemBaseName()` (helpers.js — lowercases, strips trailing counts/parentheticals/spaced-dash provenance clauses); fixed-attribute schema `{category: weapon/armor/consumable/tool/quest/treasure/mundane, effect, uses, value}` ("N/A" where inapplicable). **TYPE vs INSTANCE is a build contract**: definitions describe the TYPE; instance state (charges, provenance, counts) stays on the inventory string — an entry carrying extra fields fails the BIBLE EDITOR CONTRACT. Reads via `itemLookup()` (helpers.js): emergent overlay `worldState.itemBible` wins over the base. **Emergent items are PLAYER-CONFIRMED (ruled 2026-08-08)**: `[ITEM_DEF:]` is a PROPOSAL queued on `worldState.pendingItemDefs` (cap 5, dedupe) — the confirm modal (`showItemDefConfirmModal`, ui-modals.js; resurfaces on boot) is the only path to canon via `itemDefAccept`/`itemDefDecline` (helpers.js; accept = write-once). Injection: `buildItemBibleBlock()` (api.js, volatile) — one line per carried resolvable item across player + party sheets; mundane/treasure/effect-N/A never inject. Tooltip: `itemTip()` (ui-panels.js, closes #82's data gap). Machine-REGENERATED by bible_editor.html (canonical `JSON.stringify(x,null,2)` between the `>>> ITEM BIBLE DATA` markers, byte-pinned); read-only section in bible_study.html. **#157 (v1.583): the schema gained two DISPLAY fields** — `inventoryCategories` (non-empty array from the same seven ids, always includes `category`, serialized in registry order; the FIRST match in `INVENTORY_CATEGORY_REGISTRY` (helpers.js) decides which inventory section an item files under) and `aliases` (exact alternate TYPE names, `itemBaseName()`-normalized, collision-refused — resolution is CANONICAL inside `itemLookup`, so grouping/tooltips/GM-injection always agree; never substring/fuzzy). `effect:"N/A"` outside mundane/treasure = a legal **classification-only** entry (organizes + tooltips, never injects). Both inventory surfaces render through ONE pure `groupInventory()` view model (helpers.js — rows carry their original array index for Drop; unknown/invalid items stay VISIBLE under Unclassified, never guessed, never silently Mundane); the old substring weapon/armor panel guess is retired. Coverage audit: `dev/item-bible-coverage.js` (read-only). Spec: [DOC/DOC_inventory_reorganization.html](DOC/DOC_inventory_reorganization.html) |
| `skills_bible.js` | ✅ Active (#52) | The canonical skills reference — skill levels finally DO something specified. `SKILL_LEVEL_MECHANICS` = ONE global ladder (index-aligned with `SKILL_LEVELS`): flat check bonus per level (+1 Familiar … +5 Master, stacks with the stat mod) + auto-success bands (Trained+ stops rolling easy DC 10 routine uses when unhurried; Proficient+ even under pressure; Expert+ also moderate DC 15 unhurried; a Master rolls only for hard/contested/high-stakes) — data-only by design, a rebalance is a 6-line edit. `SKILLS_BIBLE` = per-skill canon keyed by EXACT `SKILLS` id: `def` (coverage + boundary vs confusable neighbors) + `untrained` (`yes`/`hard` = +5 DC or disadvantage/`no` = can't attempt; specialist lists derived by `skillsUntrained()`). Injection (api.js): `buildSkillMechanicsDoc()` — the constant ladder doc in the STABLE half (byte-identical, cache-safe; includes the auto-successes-never-earn-`[SKILL_SUCCESS:]` hygiene line) and `buildSkillCanonBlock()` — VOLATILE, level+bonus+stats+def for EARNED skills only, ""-clean at zero (the old `Skills (earned):` one-liner's slot). Viewer: `skillCardHTML` (helpers.js) + a Skills section in bible_study.html. Engine-tested: bidirectional coverage guard (new skill + bible entry land in the SAME commit, sabotage-proven), ladder shape, stable/volatile injection |
| `bible_editor.html` | ✅ Active (#72) | **Dev-only authoring satellite** for the `*_bible` files — separate from the read-only bible_study by design (bibles may go player-facing; players must never find a mutable surface). Opens a bible from disk and **SAVES BACK OVER IT** via the File System Access API (verified working from `file://`). Save overwrites in place behind a confirm; a lapsed handle parks and is re-granted through a **Reconnect** button inside a click gesture (FSA re-grant requires a user gesture — the #30 permission lesson); no FSA → a file input imports and Save falls back to download, loudly. **Bible types are a REGISTRY** (`BIBLE_TYPES`: detect/parse/serialize/render) — adding the item bible later is ONE entry, not new branches. Three types ship: **class_bible** (machine-regenerated wholesale), **item_bible** (#81 — machine-regenerated wholesale, the class pattern) and **capability_bible** (HAND-COMMENTED — untouched entries re-emit as their original source lines and every comment survives, so a save is a minimal diff; contract-pinned BOTH ways, unedited round-trip AND all-dirty emit). Class view: resolve badges (✓/●/○ → the fixed-6-attribute form), 234 level slots with the archetype rubric + narratability/enforceability guidance inline, spell chips, XP tab. Capability view: filterable list, per-entry edit/add/delete. Draft persists to `tnd_bible_editor_draft_v1` (v1 class-only drafts are MIGRATED, never discarded) |
...
| `map_viewer.html` | ✅ Active (#154) | Satellite viewer for the campaign's **location graph** — the GM's actual working geography, rendered as an interactive force-layout SVG (hand-rolled sim, no deps): world nodes sized by visits, road/corridor nodes as diamonds (name heuristic), sublocations clustered on their parents, first-travel edges, current party position ring, split-member badges, **ghost nodes** (edge-referenced but never filed — real rot, dashed red). Click a node → its full record (write-once description, #105 stateNotes, items placed/taken, NPCs last seen there + deceased, sublocations, connections). READ-ONLY by construction (zero localStorage writes — the recall_gate discipline). Data sources in precedence order: `window.__mapTest` seam (`load`/`graph`/`select`/`exportSVG` — headless-drivable, proven via a node DOM-stub run) → imported `.tnd`/state JSON (drag/drop or picker) → same-origin localStorage (the LIVE view beside the game on pages.dev). ⬇ Export SVG downloads the self-contained rendered map. Open directly like bible_study; network-first in sw.js (same commit, the stale-pin lesson) |
| `map_cleanup.html` | ✅ Active (#156 Phase B) | **Dev-only guided location-repair satellite** (the doc §7.4 cleanup tool; bible_editor precedent — the mutable surface lives here, map_viewer stays read-only forever). Loads a `.tnd` file OR the LIVE active campaign (same-origin localStorage via `loadState()`), runs `locRepairCensus` (typed evidence: shadow / identical-description / leaf-variant / containment; every group starts **undecided** — classification is human, per pair, never a batch, Sol §1), classify controls (merge either direction / reparent / alias / leave; **split and pipe-bearing plans ride the advanced plan-JSON box** — allocation is inherently human §7.1), **dry-run diff** of every receipt before a double-confirmed APPLY through the shipping executors (pre-images archived, reversible by construction). `.tnd` mode downloads `*_REPAIRED.tnd`; live mode writes back via `saveAll()`. Engine-subset script list + load guard; test seam `window.__cleanupTest`. Network-first in sw.js (the stale-pin lesson) |
...
| `[ITEM_DEF:name\|category=…\|effect=…\|uses=…\|value=…]` | **#81 — a PROPOSAL, never a write.** Queues a player-confirm record on `worldState.pendingItemDefs` (cap 5, dedupe vs pending AND accepted; unknown category → tool + warn; instance fields like charges/owner ignored loudly). The confirm modal is the only path to canon: Accept → write-once `worldState.itemBible[key]`, Decline → dropped loudly. Accepted canon re-injects every turn via `buildItemBibleBlock` |
