...
| `bible_editor.html` | ✅ Active (#72, v1.464) | **Dev-only authoring satellite** for class_bible.js — separate from the read-only bible_study by design. Blueprint-designer flow: edit → ⬇ export regenerated `class_bible.js` + a paste-ready `capability_additions` snippet (capability_bible.js is hand-commented, never machine-regenerated wholesale) → files land via commit, keeping the contract + coverage guards as the gate. Per-name resolve badges (✓ capability_bible / ● pending addition / ○ undefined → click opens the fixed-6-attribute define form), per-level feature editing across all 234 slots, spell chips per tier, XP-curve tab, fill-progress counter; work-in-progress persists to localStorage (`tnd_bible_editor_draft_v1`, debounced) so a closed tab loses nothing. The serializer between its `>>> BIBLE SERIALIZER` markers is THE canonical writer of class_bible.js (contract-pinned, sabotage-proven) |
