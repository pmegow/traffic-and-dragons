...
**#194 the presence split (v1.651, panel-designed, owner-ruled 2026-08-17 — [presence_panel_2026-08-17.md](DOC/Research/presence_panel_2026-08-17.md)):** a mention can never teleport a character. `[NPC:]` is REGISTRATION-ONLY (`npcRegisterMention`); presence is DERIVED at the post-handler seam from writers the GM already emits truthfully — `[SAY:]` speakers (measured 88–95% of live responses; the SAY doc line now states a tagged speaker is physically present), combat tags naming rostered NPCs, party arrivals, and engine-asked `[SCENE_CAST:]` — through `presenceObserve` into the sourced guestbook + an **evictable `frame.observed[]`** (LRU cap `PRESENCE_OBSERVED_CAP`=16, structurally unable to arm the W2 overflow latch; never mints `worldState.sceneRefs` — buildSysPrompt stays the ledger's sanctioned activator). **Gate 3 is GRADED:** witnessed limbs authorize alone under the strictly-earlier contract — living unsplit party membership, transcript speech (`entry.sp` maps within `SPEECH_EVIDENCE_TURNS`=60 of the claim; engine-written structure, not a prose scan), observed frames (non-cast), and truthful post-epoch records — while **post-epoch `statusTurn` authorizes NOTHING** (the mention channel that made 37 of 39 living t1903 NPCs killable by bare name). **Migration is one scalar** (`worldState.presenceEpoch` + `presenceVer`, stamped in migrateWorldState; grade DERIVES from turn-vs-epoch, zero records rewritten; old-client tripwire advances the epoch): pre-epoch evidence stays authorized **fail-open per ruling ③ (TENTATIVE — the flip to fail-closed is deleting the legacy clauses only; keep it that way)**, receipt-stamped `evidenceGrade:"legacy"` on canon envelopes and surfaced in the #17 standing anomalies; the window is monotonically shrinking (any mood re-stamp or re-witnessing exits it — which is why the mention-then-kill attack refuses even on grandfathered rows). **Refusal terminates:** an evidence-lack refusal arms the one-shot fork note (`buildDeathEvidenceNudge`, before the conflict nudge, capped `DEATH_EVIDENCE_NOTES`=2/subject) teaching the two honest exits — put them on the record (`[SAY:]`/cast) and re-emit, or `[NPC_DEATH_REPORTED:]` for an off-screen death; rewards then ride a new envelope via the dead-in-canon path. Cast-sourced facts (`by`/`lastSeenSrc`/observed-channel `"cast"`) are **playtest-gated out of authorization** (ruling ④) — promotion is one clause per limb after a 50-turn run measures compliance. Acceptance replay `dev/replay-t1903-presence.js` (witnessed census 37→9 with Caul cited on speech; remote-mention attack refused; legacy window 28, receipted); sabotage `dev/sabotage-presence.js` (14 clauses). Layer 6 (rumor-voice projection honesty) is a deliberate SEPARATE commit and has not shipped.
...
- **openai** — `https://api.openai.com/v1/chat/completions`; `Authorization: Bearer`; system carried as a leading `{role:"system"}` message; response at `choices[0].message.content`. Default model `gpt-5.6-sol` (sole listed model since the 2026-08-16 pruning; ⚠ gpt-5.x REJECTS `max_tokens` — buildBody sends `max_completion_tokens` for `gpt-5*`, found live at the sweep bring-up). (CORS: OpenAI allows direct browser calls, no special header.)
...
| `[SCENE_CAST:Name, Name]` / `[SCENE_CAST:none]` | #194 — engine-ASKED whole-scene cast (node change / `CAST_REFRESH_TURNS`); rostered names record cast-sourced presence at the settled node; `none` makes non-answer measurable. Parse-less on purpose (consumed at the post-handler seam); cast-sourced facts are playtest-gated OUT of death-gate authorization |
