...
| `error-report.js` | ✅ Active (#16, v1.352) | Mobile error reporting — `reportError(ctx,msg,detail)` POSTs runtime errors to a Google Apps Script webhook that emails pmegow@gmail.com (the mobile console is invisible; this is the channel). `ERROR_WEBHOOK_URL` at the top of the file (empty = disabled, one console.info). Flood control: 30s debounce (suppressed count rides the next email) + 10/session cap + reentrancy latch. Wires `window.onerror`/`onunhandledrejection` at load (browser only — node test loader skips); called from the turn/re-roll/skeleton/actions catches (game.js), summarize catch (memory.js), and the Piper narration-death crumb (tts.js). Transport seam `_erSend`/`_erPost` is test-stubbed; 15 engine tests. **#16c diagnostics (v1.407):** a per-page-load `ER_SESSION_ID` (report correlation), `erCrumb(evt,data)` — a bounded, localStorage-persisted breadcrumb ring recovered at the next boot by `erLoadPrevCrumbs()` so a PROCESS KILL's final seconds survive (no handler can run through a jetsam kill; only pre-written state does), and `erDiagBlock()` appended to every crash `detail` within a budget. All of it rides in `detail` ON PURPOSE — the GAS sheet is a fixed 15-column schema in a user-deployed script, so a new column means a redeploy + migration. **#16b user reports (v1.353):** `sendUserReport`+`erReportContext`+`ER_REPORT_HINTS` (message-keyword → extra state table; never keys/tokens) here; the modal + DOM-screenshot capture (`showBugReportModal`/`_bugCapture`, vendored `/vendor/html-to-image/` — `toSvg` only, own canvas encode; the lib's `toJpeg` hangs in embedded Chromium; since v1.365 `_bugShotFilter` drops src-less `<img>`s and `imagePlaceholder` absorbs unfetchable ones — before that ONE bad img rejected the whole toSvg and no field report ever carried a screenshot) in ui-modals.js; File ▸ ⚠ Report bug in the buildFileMenus spec (game screen only) |
...
| `ui-*.js` (×10) | ✅ Split (#54, v1.325) | ui.js (3,350 lines) decomposed per [UI_SEAM_MAP.md](DOC/UI_SEAM_MAP.md) — pure moves, byte-identical bodies, load order below. One job per file: |
...
| `tts.js` | ✅ Active | Text-to-speech: `TTS` module with a `TTS_PROVIDERS` table (mirrors the LLM `PROVIDERS` shape in globals.js) — `native` (browser speechSynthesis) / `piper` (local WASM, offline, $0 — TODO #41; vendored at `/vendor/piper/`, voices cached in OPFS after one ~60MB download). **Piper-only since the #9 rework (v1.398):** the `cartesia` cloud provider was removed and swept out on a cost/ownership call, and the engine picker went with it — selection is RESOLVED, never stored. **#90 M1 (v1.435) added the SERVER tier on top** — `TTS_PROVIDERS.server` + `TTS_LADDER = server → piper → native`: a connected player's narration synthesizes on the self-hosted **`tnd-tts` Fly app** (same 19 Piper voices, `POST /api/tts` per unit, `_speakServer` = the `_speakPiper` loop with predict()→fetch) — zero client wasm work, which is the B9 architectural close; `getEngine()` resolves `"server"` when connected+healthy, else the constant `"piper"` (offline/unconnected devices behave byte-identically to #9). Any server unit failure hands the read's remainder down the ladder via the queue + a 60s retry memo (degrade toasted once per session, D3). Self-hosting is why the #84/LiveKit cost/ownership objection doesn't apply — it's our own box; the ownership decision stands re-affirmed in [DOC/liveKit_findings.html](DOC/liveKit_findings.html). The per-item **downgrade-to-native** ladder survives below all of it — runtime availability (`_piperOk`) can drop ONE item to native without changing the engine; the v1.434 work-budget governor meters ONLY the local Piper tier (the server tier has no client work to budget) and remains the offline tier's guardian forever. Shared text-prep (`normalizeForTTS`/`splitSentences` with per-caller `dashRepl`/`commaSplit`, harvested from the piper_test.html spike at v1.298), queue/scheduler, voice bank |
| `stt.js` | ✅ Active | Speech-to-text input: `STT` module with **two independent paths**. ① **Native** — Web Speech API (`webkitSpeechRecognition`, zero-dependency, no key, no network): Chrome/Edge desktop + Android Chrome; NOT Firefox; iOS Safari has NO SpeechRecognition at all. ② **Cloud fallback (v1.342, todo_carplay rank 7)** — when native is absent AND an OpenAI key is on file, `MediaRecorder` captures the utterance (15s cap) and `whisper-1` transcribes it. **This is what makes the iPhone Safari / Car Mode path work** — the old "Chrome/Edge only" line here was stale and misled a whole LiveKit evaluation (#84). It is a fully separate code section below the native one, so native behavior stays byte-identical wherever native exists; `isSupported()` gates the mic button on EITHER path being usable. Both paths run their final transcript through **`sttCorrectNames`** (helpers.js, v1.330) — roster-phonetic correction for fantasy proper nouns (the "Frizwick → Physics" class), roster snapshotted once per listen. Prefs: language + auto-send (`tnd_stt_lang_v1`/`tnd_stt_autosend_v1`) and Car Mode auto-listen (`tnd_car_autolisten_v1`, default ON when unset). Notifies the Car Mode overlay via the global `carNotify(kind,text)` (ui-carmode.js), always behind a `typeof` guard. ⚠ **Transcription cost rides the PLAYER's own key** — that ownership property was decisive in declining LiveKit ([DOC/liveKit_findings.html](DOC/liveKit_findings.html)) |
...
| `class_bible.js` | ✅ Active (#72, v1.464 — **data only, NOT engine-wired**) | THE single definitive source for character progression once the C6 sequence lands (classDef() → store swap → `CLSS` deleted); until then the engine still reads the legacy tables and this file is loaded ONLY by the satellites + the test harness (dev/load-engine.js slots it after capability_bible.js, mirroring its eventual real position). Generated from the live engine tables at v1.464 — 9 classes, 27 archetypes, spec-R2 level grids (class 2/5/7/9/11/13/15/17 · archetype 3/6/10/14/18 + capstone 20), `CLASS_XP_LEVELS` 1–20 (L1–10 shipped verbatim, 11–20 draft), C2 spellTiers, C5 skillSeeds. 234 level slots, 36 seeded from CLASS_FEATURES with their REAL names (the `nm:"Lv9"` bug is structurally dead here), 198 fill-phase blanks. Machine-REGENERATED by bible_editor.html's exporter — canonical form `JSON.stringify(x,null,2)` between the `>>> CLASS BIBLE DATA` markers, byte-pinned by the BIBLE EDITOR CONTRACT (run-tests.js); content hand-edits in canonical form are legal, format drift fails the build. Coverage guard: every spell NAME in it must resolve via `capabilityLookup` (a new spell and its capability entry land in the SAME commit). ⚠ In sw.js's network-first regex ONLY until C6-② adds it to the app shell — remove it from the regex then |
| `bible_editor.html` | ✅ Active (#72, v1.467) | **Dev-only authoring satellite** for the `*_bible` files — separate from the read-only bible_study by design (bibles may go player-facing; players must never find a mutable surface). **v2 opens a bible from disk and SAVES BACK OVER IT** via the File System Access API (verified working from `file://`; v1's download-only flow dropped exports in ~/Downloads). Save overwrites in place behind a confirm; a lapsed handle parks and is re-granted through a **Reconnect** button inside a click gesture (the #30 permission lesson); no FSA → a file input imports and Save falls back to download, loudly. **Bible types are a REGISTRY** (`BIBLE_TYPES`: detect/parse/serialize/render) — adding the item bible later is ONE entry, not new branches. Two types ship: **class_bible** (machine-regenerated wholesale) and **capability_bible** (HAND-COMMENTED — untouched entries re-emit as their original source lines and every comment survives, so a save is a minimal diff; contract-pinned BOTH ways, unedited round-trip AND all-dirty emit, the second added after sabotage showed the first left `emit()` untested). Class view: resolve badges (✓/●/○ → the fixed-6-attribute form), 234 level slots with the archetype rubric + narratability/enforceability guidance inline, spell chips, XP tab. Capability view: filterable list, per-entry edit/add/delete. Draft persists to `tnd_bible_editor_draft_v1` (v1 class-only drafts are MIGRATED, never discarded) |
| `bug_tracker.html` | ✅ Active (#71, v1.360) | Satellite viewer for the **bug-triage pipeline** over the #16 reports. GAS v2 (`dev/gas-error-webhook.gs`, user-deployed) dual-writes every report: email (notification) + Google Sheet (record; screenshots → Drive) + secret-gated `doGet` JSON feed. Tracker = `DOC/BUGS.md` (format contract in its header — fingerprint dedup, lifecycle statuses, report bodies fenced as UNTRUSTED data); ops = the `/bugs` skill (`.claude/skills/bugs`): `sync` (feed → tracker), `investigate` (dispatches `.claude/agents/bug-investigator` — Read/Grep/Glob ONLY, mechanical injection containment), `act` (gated on findings; drift-surface flag → Fable policy). Viewer buttons COPY the /bugs command to clipboard (a page can't dispatch agents); live server section reads the doGet feed directly (secret via 🔑 → that browser's localStorage; deduped by report id against filed rows). ⚠ ALL report-derived text renders via `textContent` — never innerHTML (XSS-verified); new satellites must be added to sw.js's network-first allowlist or the SW pins them stale (the v1.360 lesson). Feed secret lives in gitignored `.claude/bugs.local.json` |
...
### 8b. RAG episodic memory (v1.154, TODO #27 Phase 1 — see [RAG_MEMORY.md](DOC/RAG_MEMORY.md))
...
**Multi-foe since UA26/v1.264** (design + ratified decisions in [MULTI_ENEMY_COMBAT.md](DOC/MULTI_ENEMY_COMBAT.md)). Combat state lives in `worldState.combat`:
...
- **`DOC/` holds reference docs — HTML and MD** (amended 2026-07-16, audit #29, from eeef396's "HTML only" rule — the least-churn reading of what the folder already held). Reference material and completed-project records (e.g. `DOC/PROJECT_ONE_DRIVE_EXODUS.html`) live there; audits live in `audits/`; satellite TOOLS (bible_study.html, blueprint-designer.html, test.html, piper_test.html) stay at root. **Enforced 2026-07-27** — the 13 reference docs that had accumulated at root (Fable_UberAudit, SERVER_ARCHITECTURE, UI_SEAM_MAP, RAG_MEMORY, MULTI_ENEMY_COMBAT, RESOLVE_NPC_INVARIANTS, STORY_COMPILER, BLUEPRINT_EDITOR, CAR_MODE, AUDIT_PLAYBOOK, PRE_REVIEW_UA30_UA31, HANDOFF_batch_v1260, FABLE_REVIEW_ACTION) moved into `DOC/`. **Four docs stay at root by design** — `CLAUDE.md`, `TODO.md`, `HANDOFF.md`, `todo_checkWithFable.md` (the daily working set; `dev/lint-todo.js` also hardcodes `../TODO.md`). Sample `.blueprint` fixtures live in **`samples/`**; personal `.tnd` saves live in **`testRuns/`** (gitignored) — a save cited by an audit must keep its link working, so move the citation in the same commit.
- **No surface is allowed to be untestable, and no guard is trusted until sabotage proves it** (2026-07-29, after a 3-hour spiral on the bible-editor save). Two standing rules and two tools:
  1. **If a surface can't be driven by `dev/run-tests.js`, the first commit that touches it adds a seam.** The editor's save path was unreachable by any harness — the file picker is an OS dialog — so four wrong fixes shipped unchallenged and the user became the test runner. The fix was a `window.__bibleTest` hook plus an **OPFS** `FileSystemFileHandle` (real handle, no picker, no gesture), which turned load→edit→save→verify-bytes into a one-second check and found a real bug on the FIRST run. Satellites with logic (`bible_editor`, `blueprint-designer`, `bug_tracker`) each need one; only the bible editor has it so far. "I can't click the button" is almost never true — buttons call functions, and functions can be called.
  2. **Prove every contract clause with `dev/sabotage.js`, and treat a mutation that changes no bytes as a FAILURE.** Three clauses shipped that day as fake coverage because the sabotage silently didn't apply (`indexOf("function _idbDel")` still matched after renaming to `_idbDelUNUSED`; `/requestPermission/` still matched the existence-check after the CALL was deleted). A green clause guarding nothing is worse than no clause. The harness restores the file on exit, crash and Ctrl-C, and asserts byte-identical restoration.
  3. **`dev/file-forensics.js <file>` before the second theory.** One command prints size · mtime/ctime · permissions · BOM · line endings · parses-or-not · tracked/HEAD-size/status · `.crswap` and temp siblings · current write-lock, and exits non-zero on anomalies. The 3-hour bug was a **zero-byte file**: Chrome kept reporting "the state had changed since it was read from disk" and was telling the literal truth, while I checked the *timestamp* four times across three theories and never the *size*. When an error asserts a fact, verify that fact exhaustively before reinterpreting it.
  4. **`dev/install-bible.js`** is the reliable bible workflow while the editor's in-place save is unproven: edit → ⬇ Download copy → `node dev/install-bible.js`. It validates (parses, 9 classes / 234 slots / 20-level XP curve, lowercase capability keys) and **refuses an empty file** — the exact failure that zeroed a tracked file.
