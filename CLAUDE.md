...
| `tag_table.js` | ✅ Active (UA1, v1.241) | ⛨ **THE tag registry** — one ordered table (`TAG_TABLE`, ~57 handlers in the exact legacy `applyMuts` order) from which three formerly hand-synced surfaces DERIVE: `applyMutsTable()` (the future parser), `buildCtTags()`/`buildCtBare()` (cleanTxt's strip regexes), and `buildStateTagsDoc()` (the STATE TAGS prompt block — byte-identical to the money-tested text, frozen by engine tests). **AUTHORITATIVE since v1.258 (cutover shipped 2026-07-10):** `applyMuts` (api.js) dispatches on `TAG_AUTHORITY` (globals, `"table"`) — the table mutates for real; the retained legacy parser (`applyMutsLegacy`) runs against cloned state as the REVERSE shadow, and any mutation diff logs loudly + persists to `tnd_tagdiff_v1` (`__tagShadowDiff`); unknown tags warn (`__tagUnknownScan`). Rollback = `TAG_AUTHORITY="legacy"` (one line — restores the pre-cutover arrangement). Adding a tag = one table entry (parse+strip+docs land together, phantom class impossible — coverage guards in the test suite). Legacy deletion is gated on the reverse-shadow soak staying clean. `TAG_NO_HANDLER` documents the deliberate parse-less names (DICE/ACTIONS/RETCON + the UA2 ENEMY_SURRENDERS phantom pending a decision). Replay tool: `dev/diff-replay.js <corpus.json>`. |
| `api.js` | ✅ Extracted | `callGM`, `buildSysPrompt`, `getRulesBlock`, `applyMuts` (v1.258: the `TAG_AUTHORITY` dispatcher — table authoritative, `applyMutsLegacy` retained as reverse shadow), `findCompanionChar`, `cleanTxt` (regexes derived from tag_table), `diceTxt`, `parseActions`, `buildGeoBlock` |
...
Entity-keyed retrieval over the verbatim transcript — no vectors, no extra API calls, **read-side only** (summarize/chapters/caps untouched). **Default ON since v1.230** (validated on the t308 mature save — RAG is what carries a long campaign through NPC-key fragmentation + the cap-30 window; see AUDIT_t308.md). Semantics: **ON unless explicitly disabled** — `ragEnabled()` returns `worldState.ragMemory!==false`, so every existing save (field unset) and every new campaign reads ON with **zero migration**; only a deliberate `false` (the Dev Mode off-switch, retained as an escape hatch) turns it off. The explicit-OFF path still reproduces the pre-RAG prompt byte-for-byte (engine-tested). Per-campaign flag rides the sync blob; toggle via **Dev Mode ▸ 🗂 Episodic memory…** (`showRagModal()`, all 3 menus).
