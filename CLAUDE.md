...
| `[ALIAS:domain\|canonical\|alias]` / `[MERGE:domain\|canonical\|duplicate]` | **#156 Phase A** — the generalized identity pair (domains: `npc`; capability/item refuse — merge=null by the TYPE ruling; location refuses until Phase B). npc operands route into the legacy handlers above (parity engine-tested). An operand containing `\|` or `]` is REFUSED loudly with no mutation (Sol §5 — pipe-bearing keys route through the Phase B cleanup tool); unknown domains refuse loudly. `[MERGE:npc\|New Name\|<provisional>]` with an absent canonical is the RENAME flow for provisional records |
