...
| `audio-events.js` · `audio-catalog.js` · `audio-profile.js` · `audio-scenes.js` · `ambient.js` · `audio-loader.js` · `audio-cache.js` · `ui-ambient.js` | ✅ General ambience first slice (L1/L7) | Engine-only SOUNDSCAPE stages bounded canonical-node metadata; save/commit publishes immutable `tnd:scene-committed` snapshots. Deterministic content-compatible catalog matching in any campaign; Village seed bindings retained. One steady bed, two during transitions; shared Sound context, narration duck/mic/pause gates. 48 MiB decoded / 8 MiB streamed caps and one manifest-versioned 32 MiB / 32-entry SW audio cache. `dev/build-audio-catalog.js` generates the committed catalog from delivery receipts; no runtime build. Scope and open gates: [proposal §20](DOC/Proposal_general_audio.html#implementation), [verification](audits/AUDIT_L7_general_audio.md). |
