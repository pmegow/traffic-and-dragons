...
| `tts.js` | ✅ Active | Text-to-speech: `TTS` module with a `TTS_PROVIDERS` table (mirrors the LLM `PROVIDERS` shape in globals.js) — `native` (browser speechSynthesis) / `piper` (local WASM, offline, $0 — TODO #41; vendored at `/vendor/piper/`, voices cached in OPFS after one ~60MB download). **Piper-only since the #9 rework (v1.398):** the `cartesia` cloud provider was removed and swept out on a cost/ownership call, and the engine picker went with it — `getEngine()` is now a CONSTANT `"piper"`, so engine *selection* no longer exists. The provider-table shape is retained so a cloud engine could return as data, but nothing cloud-shaped ships today (see TODO #84 / [DOC/liveKit_findings.html](DOC/liveKit_findings.html) for the most recent time that decision was re-tested and re-affirmed). The per-item **downgrade-to-native** ladder survives and is separate from selection — runtime availability (`_piperOk`) can drop ONE item to native without changing the engine. Shared text-prep (`normalizeForTTS`/`splitSentences` with per-caller `dashRepl`/`commaSplit`, harvested from the piper_test.html spike at v1.298), queue/scheduler, voice bank |
