...
Shared `TAG_REINFORCE` constant (globals.js) is assigned to every non-Claude provider's `reinforce` (Claude needs none). Model names in each provider's `models[]` should be verified current; the modal's dropdown is fixed to that list. **Live-test status (50-turn gemini money run, 2026-08-14/15 — [audits/AUDIT_playtest_v1635_gemini.md](audits/AUDIT_playtest_v1635_gemini.md)): tag SYNTAX and canon OBEDIENCE are separable capabilities.** gemini-3.5-flash: tag fidelity ✅ (0 zero-tag turns of 50, clean JSON extraction) but narrative coherence ✗ FAILED — its prose contradicted injected state (dead NPCs resurrected three times while the durable-dead guard kept worldState true; opening premise dropped) — NOT GM-viable without a canon-obedience `reinforce` experiment (Fable design). **gpt-4o / grok / ollama remain shape-verified only** — each still needs its live money turn.
