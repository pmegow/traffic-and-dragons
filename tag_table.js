...
var TAG_STRIP_NAMES=["HP","GOLD","ITEM_GAINED","ITEM_LOST","ITEM_KEPT","LOCATION","NPC","XP","QUEST_STEP","QUEST","DICE","COMBAT_START","COMBAT_END","COMBAT_ROUND","ENEMY_HP","ENEMY_SLAIN","ENEMY_SURRENDERS","ABILITY_GAINED","ALIGNMENT","LORE","DECISION","FUTURE_EVENT_RESOLVED","FUTURE_EVENT","NPC_NOTE","NPC_FORGET","NPC_SUPERSEDE","NPC_PRONOUN","SPELL_USED","SPELL_DEF","ITEM_DEF","MANA","SKILL_SUCCESS","CONDITION","CONDITION_REMOVED","RELATIONSHIP","RELATIONSHIP_REMOVED","SAVE_MOD","SAVE_MOD_REMOVED","LANGUAGE","STORY_BEAT","CORE_MEMORY","PARTY_MEMBER","PARTY_SPLIT","COMBAT_STATS","COMBAT_IMMUNE","COMBAT_RESIST","COMBAT_VULN","LOCATION_DESC","LOCATION_SIZE","SUBLOCATION","TIME","TIME_ADVANCE","SCHEDULE","SCHEDULE_RESOLVED","SCHEDULE_CANCEL","WEATHER","REST","LOCATION_ITEM","LOCATION_STATE","NPC_ALIAS","NPC_MERGE","NPC_LINK","FACTION","NPC_FACTION","FACTION_REL","COMPANION_HP","COMPANION_ITEM_GAINED","COMPANION_ITEM_LOST","COMPANION_ITEM_KEPT","COMPANION_SPELL_USED","COMPANION_MANA","COMPANION_XP","COMPANION_CONDITION","COMPANION_CONDITION_REMOVED","COMPANION_RELATIONSHIP","COMPANION_RELATIONSHIP_REMOVED","COMPANION_ABILITY","COMPANION_ALIGNMENT","ARC_COMPLETE","ARC_CONTINUE","ACT_COMPLETE","SAY","ACTIONS","RETCON","ALIAS","MERGE"];/* #156: the generalized identity pair — anchored \[( keeps them from shadowing NPC_ALIAS/NPC_MERGE */
...
{t:"NPC",apply:function(text,R){var npcs=text.match(/\[NPC:([^|\]]+)\|([^|\]]*)(?:\|([^|\]]*))?\]/g)||[];var ni;for(ni=0;ni<npcs.length;ni++){var np=npcs[ni].match(/\[NPC:([^|\]]+)\|([^|\]]*)(?:\|([^|\]]*))?\]/);if(!np)continue;var npName=npcUpsertTarget(np[1].trim(),(np[3]||"").trim(),R);/* #156: THE collision boundary — returns the resolved canonical (normal case, byte-identical to the old resolveNpcName call) or a provisional "Name °tN" identity when the write is a collision suspect; the whole upsert below then lands on that key, so the established record cannot fuse */
...
{t:"RELATIONSHIP_REMOVED",apply:function(text,R){var relRems=text.match(/\[RELATIONSHIP_REMOVED:([^\]]+)\]/g)||[];var rri2;for(rri2=0;rri2<relRems.length;rri2++){var rrp2=relRems[rri2].match(/\[RELATIONSHIP_REMOVED:([^\]]+)\]/);if(!rrp2)continue;if(!worldState.character.relationships)continue;var rrName=resolveNpcName(rrp2[1].trim()),rrBef=worldState.character.relationships.length;worldState.character.relationships=worldState.character.relationships.filter(function(x){return x.entity!==rrName;});var rrHit=worldState.character.relationships.length<rrBef;if(!rrHit&&typeof console!=="undefined")console.warn("[tags] RELATIONSHIP_REMOVED: no bond with '"+rrName+"' on file — nothing removed (#136⑤)");if(rrHit)R.muts.push("Rel removed: "+rrName);if(rrHit)bondToast(null,rrName,null,"ended");}}},
...
{t:"COMPANION_RELATIONSHIP",apply:function(text,R){var cRelTags=text.match(/\[COMPANION_RELATIONSHIP:([^|\]]+)\|([^|]+)\|([^\]]+)\]/g)||[];var cReli;for(cReli=0;cReli<cRelTags.length;cReli++){var cRelp=cRelTags[cReli].match(/\[COMPANION_RELATIONSHIP:([^|\]]+)\|([^|]+)\|([^\]]+)\]/);if(!cRelp)continue;var cRelCs=findCompanionChar(cRelp[1]);if(!cRelCs){if(typeof console!=="undefined")console.warn("[tags] no party member matches '"+cRelp[1].trim()+"' — companion tag dropped (#136③)");continue;}if(!cRelCs.relationships)cRelCs.relationships=[];var cRnm=resolveNpcName(cRelp[2].trim()),cRdsc=cRelp[3].trim(),cRfound=false,cRelj;for(cRelj=0;cRelj<cRelCs.relationships.length;cRelj++){if(cRelCs.relationships[cRelj].entity===cRnm){var prevCRdsc=cRelCs.relationships[cRelj].descriptor;cRelCs.relationships[cRelj].descriptor=cRdsc;cRfound=true;if(prevCRdsc!==cRdsc)bondToast(cRelp[1].trim(),cRnm,cRdsc,"updated");break;}}if(!cRfound){cRelCs.relationships.push({entity:cRnm,descriptor:cRdsc});R.muts.push(cRelp[1].trim()+": rel "+cRnm+" ("+cRdsc+")");bondToast(cRelp[1].trim(),cRnm,cRdsc,"new");}}}},
{t:"COMPANION_RELATIONSHIP_REMOVED",apply:function(text,R){var cRrTags=text.match(/\[COMPANION_RELATIONSHIP_REMOVED:([^|\]]+)\|([^\]]+)\]/g)||[];var cRri;for(cRri=0;cRri<cRrTags.length;cRri++){var cRrp=cRrTags[cRri].match(/\[COMPANION_RELATIONSHIP_REMOVED:([^|\]]+)\|([^\]]+)\]/);if(!cRrp)continue;var cRrCs=findCompanionChar(cRrp[1]);if(!cRrCs||!cRrCs.relationships){if(typeof console!=="undefined")console.warn("[tags] no party member (with relationships) matches '"+cRrp[1].trim()+"' — companion tag dropped (#136③)");continue;}var cRrNm=resolveNpcName(cRrp[2].trim()),cRrB=cRrCs.relationships.length;cRrCs.relationships=cRrCs.relationships.filter(function(x){return x.entity!==cRrNm;});if(cRrCs.relationships.length===cRrB&&typeof console!=="undefined")console.warn("[tags] COMPANION_RELATIONSHIP_REMOVED: no bond with '"+cRrNm+"' on "+cRrp[1].trim()+" (#136⑤)");if(cRrCs.relationships.length<cRrB)R.muts.push(cRrp[1].trim()+": rel removed "+cRrNm);bondToast(cRrp[1].trim(),cRrNm,null,"ended");}}},
