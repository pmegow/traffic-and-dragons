...
function awardMilestoneXp(kind,label,R){
  ...
  /* audit 2026-09-18 A1: the kind's xp:"none" gated only the GM's [XP:] — the engine paymaster paid quests, bosses and
     acts in the village regardless, and mirrored the award to every companion. One gate, via the kind, on every payer. */
  if(typeof kindDef==="function"&&kindDef().xp==="none"){if(R&&R.muts)R.muts.push("XP refused — the village pays nothing ("+kind+" milestone: "+label+")");if(typeof console!=="undefined")console.warn("[tags] milestone XP refused in the village — the village pays nothing: "+kind+" "+label);return 0;}
...
var TAG_DOC_LINES=[
"[SOUNDSCAPE:place|enclosure=open;setting=wilderness;biome=temperate;quiet=normal;allows=birds,insects,wind;forbid=none] -- cosmetic classification only when the engine asks; no hidden facts, URLs or file names\n",
...
"[LAYOUT:room|size|features|opens onto; room|size|features|opens onto; ...] -- the ROOM GRAPH of the current place, filed ONCE when the engine asks: one entry per room -- its name, a size word (tiny/small/medium/large), its fixed features in a few words, and the rooms it opens onto (comma-separated; 'outside' for a door to the open air). No coordinates. Every connection must name a listed room or 'outside'. The engine serves it back every turn you are inside, and it is canon: never narrate a room or a door the record does not hold\n",
...
"[COMPANION_GROWTH:Name|the flaw as written on their sheet|what replaced it] -- RARE: the companion acted AGAINST their flaw at a defining moment, on screen and unasked (the spider-fearing companion who drags the party out of the web). The sheet's flaw is rewritten and the old one kept. Never file growth because the engine or the player asked for it. [COMPANION_GROWTH:Name|motivation|settled: how it ended] -- RARE: the companion's standing motivation was fulfilled ON SCREEN (the creditor found, the brother freed); it retires into their history and the sheet reads none standing until the story births another. [COMPANION_GROWTH:Name|motivation|the new purpose in one sentence] -- RARE: a defining moment gave a companion a NEW purpose (or replaced the standing one, which is kept). Never a debt, ledger, contract, toll or paperwork purpose -- the engine refuses those; debts here are blood, oaths, hunger and memory\n",/* #370; #437 the motivation forms */
...
"[SAY:Character Name] or [SAY:Character Name|mood] -- VOICE ATTRIBUTION: place immediately BEFORE every line of spoken dialogue, naming its speaker, e.g. [SAY:Frizwick]\"Don't jinx it,\" Frizwick mutters. Tag EVERY quoted line -- including the player character's own lines (use their character NAME, never 'you'). Tag EACH NEW PARAGRAPH of a continuing speech too -- a paragraph-opening quote always takes a fresh tag; an untagged one falls to the narrator. Use the speaker's exact registered name; omit the tag only for unnamed incidental speakers. The tag is invisible to the player and tells the narrator engine which voice performs the line -- an untagged line is read in the narrator's voice. A tagged speaker is understood to be PHYSICALLY PRESENT in the scene you are narrating -- if a voice is remote or unreal (a letter read aloud, a sending, a scrying, a remembered line, a dream), do not tag it; render it in the narrator's voice. The optional |mood is HOW the line is spoken -- a short phrase (letters, spaces, commas; at most 40 characters; never a sentence) that the voice engine performs for that line and for the lines that follow it until the next tag. Use it ONLY when the feeling is not obvious from the words themselves, and never on every line -- most lines carry none. Emotions: happy, sad, angry, surprised, terrified, concerned, amused, weary, cold, tender. Delivery: whispering, urgent, slow and measured, low and gravelly, bright, flat. Non-verbal sounds performed before the line: laugh, giggle, sigh, breathe, clear throat, cough, yawn. Combine with a comma, e.g. [SAY:Frizwick|sound concerned, speaking carefully]\"Don't jinx it,\" Frizwick mutters. Never pair opposites (a whisper with loud).\n",
...
var TAG_DOC_ENGINE_ONLY=["SOUNDSCAPE","NPC_SUPERSEDE","NPC_MERGE","ALIAS","MERGE","ITEM_RENAMED","COMPANION_ITEM_RENAMED","WHISPER","LAYOUT","EXIT","DEATH_ANSWER","ITEM_KEPT","COMPANION_ITEM_KEPT"];/* audit A13: the three note-taught tags that sat outside the #311 phantom check *//* #415: EXIT is taught by the geo block's standing volatile line — a door is filed at narration time, not on request *//* #408: taught by buildLayoutNote where a record is wanted; the standing doc stays byte-identical */
...
{t:"COMPANION_GROWTH",apply:function(text,R){var gs=text.match(/\[COMPANION_GROWTH:([^|\]]+)\|([^|\]]+)\|([^\]]+)\]/g)||[],gi;for(gi=0;gi<gs.length;gi++){var gm=gs[gi].match(/\[COMPANION_GROWTH:([^|\]]+)\|([^|\]]+)\|([^\]]+)\]/);if(!gm)continue;var gnm=resolveNpcName(gm[1].trim()),gcs=findCompanionChar(gnm);
  ...
  if(/^motivation$/i.test(gm[2].trim())){/* #437: the motivation axis — settle or birth; the flaw path below is untouched */
    ...
    if(MOTIVATION_SETTLED_RE.test(mv)){var mHow=mv.replace(MOTIVATION_SETTLED_RE,"").trim()||"settled";var ms=motivationSettle(gcs,mHow,R.turn,mCamp);
      if(!ms){if(typeof console!=="undefined")console.warn("[growth] "+gname+" has nothing standing to settle — the motivation is already empty (#437)");R.muts.push("⚠ "+gname+": nothing standing to settle");continue;}
      R.muts.push(gname+": purpose settled — "+ms.how);if(typeof fileCoreMemory==="function")fileCoreMemory("growth",gname,gname+"'s purpose was settled: "+ms.how);
...
{t:"COMPANION_AGENDA_BEAT",apply:function(text,R){var ts=text.match(/\[COMPANION_AGENDA_BEAT:([^\]]+)\]/g)||[],i;for(i=0;i<ts.length;i++){var m=ts[i].match(/\[COMPANION_AGENDA_BEAT:([^\]]+)\]/);if(!m)continue;var cs=findCompanionChar(resolveNpcName(m[1].trim()));if(cs&&cs.agenda){cs.agenda.lastBeat=(typeof clockNow==="function")?clockNow():0;cs.agenda.beats=(cs.agenda.beats||0)+1;}else{if(typeof console!=="undefined")console.warn("[agenda] [COMPANION_AGENDA_BEAT:"+m[1].trim()+"] — "+(cs?"no want on record":"no companion sheet by that name")+"; ignored (#136③)");R.muts.push("⚠ [COMPANION_AGENDA_BEAT:] "+m[1].trim()+" — "+(cs?"no want on record":"no companion by that name")+" (dropped)");}}}},
...
{t:"GOLD",apply:function(text,R){var goldTags=text.match(/\[GOLD:\s*([+-]?\d+)[^\]]*\]/g)||[];var gli;
  /* #6 F5: THE trade gate (owner, 2026-09-12: buy/sell only in a shop, with a named counterparty). In a tradeOnlyInShops kind
     coin moves only while the hero stands in a shop with the keeper present; otherwise every [GOLD:] in the response is refused
     by name and the items riding with it (ITEM_GAINED/ITEM_LOST below, ordered after) are refused with it. One gate, via the kind. */
  if(goldTags.length&&typeof kindDef==="function"&&kindDef().tradeOnlyInShops&&typeof villageTradeContext==="function"){var _vtc=villageTradeContext(text);if(!_vtc.ok){R.villageTradeRefused=_vtc.reason;worldState.tradeRefusedPing={turn:R.turn,reason:_vtc.reason,items:/\[ITEM_(GAINED|LOST):/.test(text)};/* #6 F9: the channel */R.muts.push("Trade refused — "+_vtc.reason+" (kind: village): coin changes hands only in a shop, with the keeper named");if(typeof console!=="undefined")console.warn("[tags] GOLD refused in the village — "+_vtc.reason+" (no shop or no counterparty): "+goldTags.join(" "));return;}}for(gli=0;gli<goldTags.length;gli++){var glm=goldTags[gli].match(/\[GOLD:\s*([+-]?\d+)[^\]]*\]/);if(!glm)continue;var dg=parseInt(glm[1]);worldState.character.gold=Math.max(0,worldState.character.gold+dg);R.muts.push(dg>0?"+"+dg+" gp":dg+" gp");}}},
{t:"ITEM_GAINED",apply:function(text,R){var igTags=text.match(/\[ITEM_GAINED:([^\]]+)\]/g)||[],igCounts={},igi;
  if(R.villageTradeRefused&&igTags.length){R.muts.push("Trade refused — nothing gained ("+igTags.length+" item tag"+(igTags.length>1?"s":"")+" rode with the refused coin)");if(typeof console!=="undefined")console.warn("[tags] ITEM_GAINED refused with the village trade: "+igTags.join(" "));return;}/* #6 F5 */for(igi=0;igi<igTags.length;igi++){var ig0=igTags[igi].match(/\[ITEM_GAINED:([^\]]+)\]/);if(ig0){var iq0=_qtyParse(ig0[1]),ik0=itemBaseName(iq0.base);igCounts[ik0]=(igCounts[ik0]||0)+iq0.n;}}for(igi=0;igi<igTags.length;igi++){var igm=igTags[igi].match(/\[ITEM_GAINED:([^\]]+)\]/);if(!igm)continue;var igq=_qtyParse(igm[1]),igqi;duplicateItemGrantWarning(worldState.character.inventory,igq.base,igCounts[itemBaseName(igq.base)],null,R,text);for(igqi=0;igqi<igq.n;igqi++)addInventoryItem(worldState.character.inventory,igq.base);R.muts.push("+"+igq.base+(igq.n>1?" x"+igq.n:""));var _at=autoTakeLocationItem(igq.base);if(_at&&_at.kept){R.muts.push("Stash kept — "+_at.name+" stays in "+_at.owner+"'s house (only its owner takes from a house)");if(typeof console!=="undefined")console.warn("[tags] auto-take gated in the village: "+_at.name+" belongs to "+_at.owner+"'s house (#6 E5)");}_itemDefCandidate(igq.base);}}},
{t:"ITEM_LOST",apply:function(text,R){var ilTags=text.match(/\[ITEM_LOST:([^\]]+)\]/g)||[];var ili;
  if(R.villageTradeRefused&&ilTags.length){R.muts.push("Trade refused — nothing lost ("+ilTags.length+" item tag"+(ilTags.length>1?"s":"")+" rode with the refused coin)");if(typeof console!=="undefined")console.warn("[tags] ITEM_LOST refused with the village trade: "+ilTags.join(" "));return;}/* #6 F5 */for(ili=0;ili<ilTags.length;ili++){var ilm=ilTags[ili].match(/\[ITEM_LOST:([^\]]+)\]/);if(!ilm)continue;var ilq=_qtyParse(ilm[1]),ilqi,ilHit=0;for(ilqi=0;ilqi<ilq.n;ilqi++){if(removeInventoryItem(worldState.character.inventory,ilq.base))ilHit++;}if(ilHit){R.muts.push("-"+ilq.base+(ilHit>1?" x"+ilHit:""));_clearConsumablePending(null,ilq.base);if(typeof wornPrune==="function")wornPrune(worldState.character);/* #388: nothing is worn that is not carried */}else if(typeof console!=="undefined")console.warn("[tags] ITEM_LOST: no inventory entry matches '"+ilq.base+"' — nothing removed, no receipt minted (#136⑤, the live t1530 -none class)");}}},
...
/* #6F8 (2026-09-12): TEXT order, not table order — a response that leaves one sub-location and arrives at another ends at
   ...
   end at null (live t8: the shop shelf was refused because the arrival had been erased). A leave that follows the last
   arrival, or stands alone, still clears. */
{t:"SUBLOCATION_LEAVE",apply:function(text,R){var _lv=text.lastIndexOf("[SUBLOCATION_LEAVE]");if(_lv<0)return;var _arLast=-1,_m=text.match(/\[SUBLOCATION:[^\]]+\]/g);if(_m&&_m.length)_arLast=text.lastIndexOf(_m[_m.length-1]);if(_arLast>_lv)return;/* the arrival came after the leave — it stands */worldState.world.sublocation=null;R.muts.push("Left sub-location");}},
...
/* #6 phase E: a third operand names the place the item files to ([LOCATION_ITEM:name|placed|Frizwick's house]); every
   refusal (missing node, `taken` in a kind whose stash has no actor) is LOUD — mutation log + console — never a silent
   drop; the last move is recorded for Car Mode's spoken undo (undoLastItemMove, game.js). */
/* #408 ①: the room graph — [LAYOUT:room|size|features|to; room|…] files a place's rooms and how they connect on the
   CURRENT node (ordered after LOCATION/SUBLOCATION so it lands on the new node). ENGINE-ONLY tier: the standing doc never
   carries it; the #408 ② ask note teaches it where a record is wanted. Every refusal is loud and files nothing. */
{t:"LAYOUT",apply:function(text,R){var lts=text.match(/\[LAYOUT:([^\]]*)\]/g)||[],li;for(li=0;li<lts.length;li++){var lb=lts[li].slice(8,-1);var lr=(typeof fileLayout==="function")?fileLayout(lb,R.turn,"gm",(typeof lastAction==="string")?lastAction:""):{ok:false,reason:"fileLayout missing"};/* #408 ③: the player's own last action is the voice gate's evidence */
  if(!lr.ok){R.muts.push("⚠ [LAYOUT:] refused — "+lr.reason);if(typeof console!=="undefined")console.warn("[tags] LAYOUT refused at "+(lr.key||"the current node")+": "+lr.reason+" — nothing filed (#408)");continue;}
  R.muts.push((lr.voiced?"Layout changed at the player's word: ":"Layout: ")+lr.rooms+" room"+(lr.rooms>1?"s":"")+" on record");}}},
/* #415: the narrated door — [EXIT:name|note] files a way the GM described that has no place on record, write-once on the
   CURRENT node (after LOCATION/SUBLOCATION so it lands on the new node). ENGINE-ONLY tier: the geo block's UNEXPLORED WAYS
   line teaches it every turn. Every refusal is loud and files nothing; a duplicate is a benign skip; the note is display only. */
{t:"EXIT",apply:function(text,R){var exs=text.match(/\[EXIT:([^\]]*)\]/g)||[],xi;for(xi=0;xi<exs.length;xi++){var xb=exs[xi].slice(6,-1),xp=xb.indexOf("|"),xn=xp<0?xb:xb.slice(0,xp),xt=(xp<0?"":xb.slice(xp+1)).trim();
  var xr=(typeof fileExit==="function")?fileExit(xn,xt,R.turn):{ok:false,reason:"fileExit missing"};
  if(!xr.ok){R.muts.push("⚠ [EXIT:] refused — "+xr.reason);if(typeof console!=="undefined")console.warn("[tags] EXIT refused at "+(xr.key||"the current node")+": "+xr.reason+" — nothing filed (#415)");continue;}
  if(xr.dup){if(typeof console!=="undefined"&&console.debug)console.debug("[tags] EXIT '"+xr.name+"' already on record at "+xr.key+" — skipped (#415)");continue;}
  R.muts.push("Exit: "+xr.name+(xt?" ("+xt+")":"")+" — "+xr.open+" of "+EXIT_CAP+" open ways here");}}},
/* #408 ④: a fourth operand pins the placed row to a room ([LOCATION_ITEM:name|placed|<place>|<room, spot>]) — additive, never a rename. */
{t:"LOCATION_ITEM",apply:function(text,R){__tagNearMiss(text,R,"LOCATION_ITEM","^\\[LOCATION_ITEM:[^|\\]]+\\|(placed|taken)(\\|[^|\\]]+)?(\\|[^|\\]]+)?\\]$","[LOCATION_ITEM:name|placed-or-taken]");var locItms=text.match(/\[LOCATION_ITEM:([^|\]]+)\|(placed|taken)(?:\|([^|\]]+))?(?:\|([^|\]]+))?\]/g)||[];var lii;for(lii=0;lii<locItms.length;lii++){var lip=locItms[lii].match(/\[LOCATION_ITEM:([^|\]]+)\|(placed|taken)(?:\|([^|\]]+))?(?:\|([^|\]]+))?\]/);if(!lip)continue;var _lnm=lip[1].trim(),_lact=lip[2],_lplace=lip[3]?lip[3].trim():null,_lroom=(lip[4]&&_lact==="placed")?lip[4].trim():null;
  if(_lact==="taken"&&typeof kindDef==="function"&&kindDef().stashQuantities){R.muts.push("Stash refused — [LOCATION_ITEM:"+_lnm+"|taken] names no actor; nothing leaves a stash but by its owner's hand (kind: village)");if(typeof console!=="undefined")console.warn("[tags] LOCATION_ITEM taken refused in the village (no actor): "+_lnm);continue;}
  var _lst=fileLocationItem(_lnm,_lact,R.turn,_lplace,_lroom);
  if(!_lst||!_lst.ok){var _lwhy=(_lst&&_lst.reason)||"unknown";R.muts.push("Stash refused — "+_lnm+" ("+(_lplace||"here")+"): "+_lwhy);if(typeof console!=="undefined")console.warn("[tags] LOCATION_ITEM "+_lact+" refused for '"+_lnm+"' at "+(_lplace||"the current node")+": "+_lwhy+" — nothing mutated");continue;}
  if(_lst.roomRefused)R.muts.push("⚠ room refused for "+_lnm+" — "+_lst.roomRefused);/* #408 ④ */
  worldState.lastItemMove={name:_lnm,action:_lact,key:_lst.key,turn:R.turn};
  R.muts.push((_lact==="placed"?"Left: ":"Taken: ")+_lnm+(_lst.qty>1?" ×"+_lst.qty:"")+(_lplace?" — at "+_lplace:"")+((_lroom&&!_lst.roomRefused)?" — "+_lroom:""));}}},
...
{t:"WARES",apply:function(text,R){var _wraw=text.match(/\[WARES:[^\]]*\](?:\|[^\[\]\n]*\])*/g)||[],wt=[],_wri;
  ...
  for(_wri=0;_wri<_wraw.length;_wri++){var _wseg=_wraw[_wri].slice(7,-1).split("]|"),_wsi;if(_wseg.length>1&&typeof console!=="undefined")console.warn("[wares] chained WARES form accepted ("+_wseg.length+" wares in one tag) — the contract is one [WARES:item|price|note] per ware");for(_wsi=0;_wsi<_wseg.length;_wsi++)wt.push("[WARES:"+_wseg[_wsi]+"]");}
  var wi;for(wi=0;wi<wt.length;wi++){var body=wt[wi].slice(7,-1).trim();if(/^none$/i.test(body)){if(fileWaresNone(R.turn))R.muts.push("Market: nothing for sale here");continue;}var parts=body.split("|");if(parts.length<2||!parts[0].trim()||!parts[1].trim()){if(typeof console!=="undefined")console.warn("[wares] [WARES:"+body+"] refused — the form is [WARES:item|price|note] or [WARES:none] (#303)");continue;}
  if(typeof kindDef==="function"&&kindDef().waresPerShop&&!waresNodeFor(R.turn)){R.muts.push("Wares refused — "+parts[0].trim()+": not in a shop (kind: village); wares file only from inside the shop that sells them");if(typeof console!=="undefined")console.warn("[wares] [WARES:"+body+"] refused in the village — the hero is not in a shop sub-location (#6 F2)");continue;}
  var _wout={},row=fileWare(parts[0],parts[1],parts.slice(2).join("|"),R.turn,_wout);if(!row)continue;var rec="For sale: "+row.item+" — "+row.price;
  if(_wout.pinned)rec+=" (pinned to canon; quoted "+_wout.pinned.from+")";else if(_wout.anchored)rec+=" (first quote kept; re-quoted "+_wout.anchored.quoted+")";
  if(_wout.evicted&&_wout.evicted.length)R.muts.push("Shelf full — dropped: "+_wout.evicted.join(", ")+" (this shop holds "+WARES_CAP_SHOP+")");var canon=(typeof itemLookup==="function")?itemLookup(row.item):null;var cg=(typeof itemValueGp==="function")?itemValueGp(canon):null;var pg=(typeof itemValueGp==="function")?itemValueGp({value:row.price}):null;var band=(typeof WARES_PRICE_BAND==="number")?WARES_PRICE_BAND:3;if(cg&&pg&&(pg>cg*band||pg<cg/band)){if(typeof console!=="undefined")console.warn("[wares] "+row.item+" priced "+row.price+" against a bible value of "+cg+" gp — outside the "+band+"× band; the narrated price stands, canon shown beside it (#303)");rec+=" (canon "+cg+" gp)";}R.muts.push(rec);}}},
{t:"WANTED",apply:function(text,R){var wt=text.match(/\[WANTED:([^\]]*)\]/g)||[];var wi;for(wi=0;wi<wt.length;wi++){var parts=wt[wi].slice(8,-1).split("|");if(parts.length<2||!parts[0].trim()){if(typeof console!=="undefined")console.warn("[wanted] "+wt[wi]+" refused — the form is [WANTED:item|offer|by] (#303)");continue;}var row=fileWanted(parts[0],parts[1],parts[2],R.turn);if(row)R.muts.push("Wanted here: "+row.item+(row.by?" (by "+row.by+")":""));if(row&&fileWanted.lastEvicted&&fileWanted.lastEvicted.length)R.muts.push("Wanted evicted (cap "+((typeof WANTED_CAP!=="undefined")?WANTED_CAP:4)+"): "+fileWanted.lastEvicted.join(", "));/* audit C8 */}}},
...
{t:"NPC_MERGE",apply:function(text,R){var npcMergeTags=text.match(/\[NPC_MERGE:([^|\]]+)\|([^\]]+)\]/g)||[];var mgii;for(mgii=0;mgii<npcMergeTags.length;mgii++){var mgp=npcMergeTags[mgii].match(/\[NPC_MERGE:([^|\]]+)\|([^\]]+)\]/);if(!mgp)continue;var mgCanon=mgp[1].trim(),mgDupe=mgp[2].trim();var _imWs=wsNpcByName(mgDupe);if(memory.npcs[mgDupe]||_imWs){/* #156: complete pre-image BEFORE any mutation — every identity merge is reversible by construction (P12); rides the .tnd import whitelist. #272 D5 (f44b): hoisted ABOVE the memory branch — a worldState-only duplicate used to fold with NO pre-image at all; portraits ride as honest markers via _mgPreImageWs. */memArchive().identityMerges.push({domain:"npc",canonical:mgCanon,duplicate:mgDupe,turn:R.turn,records:{mem:memory.npcs[mgDupe]?JSON.parse(JSON.stringify(memory.npcs[mgDupe])):null,ws:_mgPreImageWs(_imWs)}});}if(memory.npcs[mgDupe]){if(!memory.npcs[mgCanon])memory.npcs[mgCanon]={attitude:"unknown",knowledge:[],events:[],aliases:[]};if(!memory.npcs[mgCanon].aliases)memory.npcs[mgCanon].aliases=[];if(!npcIsProvisional(mgDupe)&&memory.npcs[mgCanon].aliases.indexOf(mgDupe)<0)memory.npcs[mgCanon].aliases.push(mgDupe);/* #156: a provisional ° key never recurs in prose — registering it as a permanent alias would only pollute the table */var mgevs=memory.npcs[mgDupe].events||[],mgevi;for(mgevi=0;mgevi<mgevs.length;mgevi++)memory.npcs[mgCanon].events.push(mgevs[mgevi]);var _mgAuth=memory.npcs[mgDupe].authored||[],_mgAi;for(_mgAi=0;_mgAi<_mgAuth.length;_mgAi++)fileNpcAuthored(memory.npcs[mgCanon],_mgAuth[_mgAi].text);var _mgSecrets=memory.npcs[mgDupe].secrets||[],_mgSi;for(_mgSi=0;_mgSi<_mgSecrets.length;_mgSi++){var _mgS=_mgSecrets[_mgSi];if(_mgS)fileNpcSecret(memory.npcs[mgCanon],_mgS.text,_mgS.revealAct);}var mgkns=memory.npcs[mgDupe].knowledge||[],mgkni;for(mgkni=0;mgkni<mgkns.length;mgkni++){if(memory.npcs[mgCanon].knowledge.indexOf(mgkns[mgkni])<0)memory.npcs[mgCanon].knowledge.push(mgkns[mgkni]);}if(memory.npcs[mgCanon].knowledge.length>12){var mgOv=memory.npcs[mgCanon].knowledge.splice(0,memory.npcs[mgCanon].knowledge.length-12),mgOvi;for(mgOvi=0;mgOvi<mgOv.length;mgOvi++)memArchive().npcKnowledge.push({npc:mgCanon,fact:mgOv[mgOvi],turn:worldState.turn});if(typeof console!=="undefined")console.warn("[memory] #144A: merge truncated "+mgOv.length+" older facts on "+mgCanon+" — archived, not destroyed (the t1265→t1549 Morwen class: ~36 facts across 3 keys silently cut to 12)");R.muts.push("Merge: "+mgOv.length+" older facts on "+mgCanon+" archived");}/* re-slice to the write-site cap after a merge concat (E50 parallel) — pre-#144A this slice DESTROYED the overflow at exactly the moment two half-histories finally united */if(memory.npcs[mgDupe].aliases){var mgals=memory.npcs[mgDupe].aliases,mgali;for(mgali=0;mgali<mgals.length;mgali++){if(memory.npcs[mgCanon].aliases.indexOf(mgals[mgali])<0)memory.npcs[mgCanon].aliases.push(mgals[mgali]);}}if(!memory.npcs[mgCanon].firstEncounter&&memory.npcs[mgDupe].firstEncounter)memory.npcs[mgCanon].firstEncounter=memory.npcs[mgDupe].firstEncounter;if(memory.npcs[mgDupe].dead&&!memory.npcs[mgCanon].dead)memory.npcs[mgCanon].dead=memory.npcs[mgDupe].dead;/* B3: a merge must not lose the dupe's death */
  /* audit 2026-09-18 C9: the presence stamps and the memory-side pronouns/attitude were the fields the fold forgot — the
     W2 "recorded at the party's current location" limb and the witnessed presence tier lost the sighting the duplicate key
     held. Newer sighting wins; pronouns/attitude: canonical wins, the dupe's fills a blank. */
  (function(_mc,_md){var _mdT=(typeof _md.lastSeenTurn==="number")?_md.lastSeenTurn:-1,_mcT=(typeof _mc.lastSeenTurn==="number")?_mc.lastSeenTurn:-1;
    if(_mdT>_mcT){_mc.lastSeenTurn=_md.lastSeenTurn;if(_md.lastSeenAt!==undefined)_mc.lastSeenAt=_md.lastSeenAt;if(_md.lastSeenSrc!==undefined)_mc.lastSeenSrc=_md.lastSeenSrc;}
    if(typeof _md.lastMentioned==="number"&&!(typeof _mc.lastMentioned==="number"&&_mc.lastMentioned>=_md.lastMentioned))_mc.lastMentioned=_md.lastMentioned;
    if(!_mc.pronouns&&_md.pronouns)_mc.pronouns=_md.pronouns;if(!_mc.attitude&&_md.attitude)_mc.attitude=_md.attitude;})(memory.npcs[mgCanon],memory.npcs[mgDupe]);
...
{t:"NPC",apply:function(text,R){var npcs=text.match(/\[NPC:([^|\]]+)\|([^|\]]*)(?:\|([^|\]]*))?\]/g)||[];var ni;for(ni=0;ni<npcs.length;ni++){var np=npcs[ni].match(/\[NPC:([^|\]]+)\|([^|\]]*)(?:\|([^|\]]*))?\]/);if(!np)continue;var _npRaw=np[1].trim(),_npResolved=resolveNpcName(_npRaw);if(typeof memoryNpcIsPlayer==="function"&&memoryNpcIsPlayer(_npResolved)){if(typeof console!=="undefined")console.warn("[tags] NPC write for player '"+_npRaw+"' refused — player identity belongs to the character sheet");R.muts.push("NPC write refused (player): "+_npRaw);continue;}var npName=npcUpsertTarget(_npRaw,(np[3]||"").trim(),R);/* #156: THE collision boundary — returns the resolved canonical or a provisional identity when the write is a collision suspect. */
  ...
  if(_npN){if(npRel)_npN.rel=npRel;if(npPron)_npN.pronouns=npPron;
    ...
    /* audit 2026-09-18 A4: the Pax gate covered HP, COMPANION_HP and the reported death, but not the bare [NPC:name|dead|…]
       the standing doc teaches — the common emission path was the ungated one. Refuse the death (the NPC stays registered
       and alive; the dead phrase never lands as a status either), name the goddess in the log. */
    if(npStatus&&!_npN.dead&&npcDeadStatus(npStatus)&&__villageHarmRefused(R,"NPC death",npName+"|"+npStatus)){npStatus="";if(_npN.status&&npcDeadStatus(_npN.status)){_npN.status="";_npN.statusTurn=0;}}
...
{t:"XP",apply:function(text,R){/* #302: the GM's [XP:] is FLAVOUR — summed per response and clamped to GM_XP_CAP_PER_LEVEL × level; quests, bosses and acts are paid by awardMilestoneXp at their own seams. The clamp is loud on both channels, never silent. */var xpTags=text.match(/\[XP:\s*\+?(\d+)[^\]]*\]/g)||[];var xpi,_xpSum=0;
  if(xpTags.length&&typeof kindDef==="function"&&kindDef().xp==="none"){R.muts.push("XP refused — the village pays nothing (kind: village)");if(typeof console!=="undefined")console.warn("[tags] XP refused in the village — the village pays nothing: "+xpTags.join(" "));return;}/* #6 H1 */for(xpi=0;xpi<xpTags.length;xpi++){var xpm=xpTags[xpi].match(/\[XP:\s*\+?(\d+)[^\]]*\]/);if(!xpm)continue;_xpSum+=parseInt(xpm[1]);}if(!_xpSum)return;var _xpCap=(typeof GM_XP_CAP_PER_LEVEL==="number"?GM_XP_CAP_PER_LEVEL:10)*Math.max(1,worldState.character.level||1);if(_xpSum>_xpCap){if(typeof console!=="undefined")console.warn("[xp] GM award "+_xpSum+" XP clamped to "+_xpCap+" ("+GM_XP_CAP_PER_LEVEL+"× level; quests, bosses and acts are engine-paid — #302)");R.muts.push("+"+_xpCap+" XP (GM award "+_xpSum+" clamped to "+_xpCap+")");_xpSum=_xpCap;}else R.muts.push("+"+_xpSum+" XP");worldState.character.xp+=_xpSum;checkLevelUp();R._xpMirror(_xpSum);}},
...
{t:"COMBAT_START",apply:function(text,R){
  ...
  /* audit A6: the highest-stakes strict shape in the table was not enrolled in the near-miss tripwire — a malformed start
     opened no fight and said nothing, and every later [ENEMY_HP:] then no-oped. */
  __tagNearMiss(text,R,"COMBAT_START","^\\[COMBAT_START:[^|\\]]+\\|\\d+\\|\\d+\\|[+-]?\\d+\\|[^|\\]]+\\|[^\\]]+\\]$","[COMBAT_START:name|hp|ac|atk|dmg|morale] — hp, ac and atk are plain integers");
...
{t:"SPELL_DEF",apply:function(text,R){var spellDefs=text.match(/\[SPELL_DEF:([^\]]+)\]/g)||[];var sdi;for(sdi=0;sdi<spellDefs.length;sdi++){
  ...
  /* audit A2: never file an EMPTY canon — write-once means an empty definition served forever and priced casts off tier 0
     ...
     refused loud; the GM re-emits it in key=value form next turn. */
  if(!sdEntry.range&&!sdEntry.targets&&!sdEntry.duration&&!sdEntry.effect){if(typeof console!=="undefined")console.warn("[tags] SPELL_DEF: '"+sdName+"' carried no key=value fields (positional form is not read) — REFUSED, nothing filed (audit A2)");R.muts.push("⚠ Spell canon NOT defined: "+sdName+" — no key=value fields (write range=…|targets=…|duration=…|effect=…)");continue;}
...
{t:"SAVE_MOD",apply:function(text,R){__tagNearMiss(text,R,"SAVE_MOD","^\\[SAVE_MOD:[^|\\]]+\\|[^|\\]]+\\|\\s*[+-]?\\d+\\s*\\]$","[SAVE_MOD:source|threat|+N/-N] — the modifier is a plain integer");/* audit A6 */var saveTags=text.match(/\[SAVE_MOD:([^|\]]+)\|([^|]+)\|([^\]]+)\]/g)||[];var smi2;for(smi2=0;smi2<saveTags.length;smi2++){var smp2=saveTags[smi2].match(/\[SAVE_MOD:([^|\]]+)\|([^|]+)\|([^\]]+)\]/);if(!smp2)continue;if(!worldState.character.saveModifiers)worldState.character.saveModifiers=[];var ssrc=smp2[1].trim(),stype=smp2[2].trim(),sval=parseInt(smp2[3]);if(isNaN(sval))continue;var sfound=false,smj;for(smj=0;smj<worldState.character.saveModifiers.length;smj++){if(worldState.character.saveModifiers[smj].source===ssrc){worldState.character.saveModifiers[smj].type=stype;worldState.character.saveModifiers[smj].amount=sval;sfound=true;break;}}if(!sfound)worldState.character.saveModifiers.push({source:ssrc,type:stype,amount:sval});var svalStr=sval>=0?"+"+sval:""+sval;R.muts.push("Save "+svalStr+" vs "+stype+" ["+ssrc+"]");}}},
...
{t:"COMPANION_ITEM_GAINED",apply:function(text,R){var cIgTags=text.match(/\[COMPANION_ITEM_GAINED:([^|\]]+)\|([^\]]+)\]/g)||[],cIgCounts={},cIgi;
...
{t:"COMPANION_XP",apply:function(text,R){var cXpTags=text.match(/\[COMPANION_XP:([^|\]]+)\|\s*\+?(\d+)[^\]]*\]/g)||[];var cXpi;for(cXpi=0;cXpi<cXpTags.length;cXpi++){var cXpm=cXpTags[cXpi].match(/\[COMPANION_XP:([^|\]]+)\|\s*\+?(\d+)[^\]]*\]/);if(!cXpm)continue;/* audit A1: the kind's xp:none gates the companion payer too */if(typeof kindDef==="function"&&kindDef().xp==="none"){R.muts.push("XP refused — the village pays nothing ("+cXpm[1].trim()+")");if(typeof console!=="undefined")console.warn("[tags] COMPANION_XP refused in the village — the village pays nothing: "+cXpTags[cXpi]);continue;}
  var cXpNpc=findCompanionNpc(cXpm[1]);if(!cXpNpc||!cXpNpc.charSheet){/* audit A7: a miss is loud (the #136③ house style every other COMPANION_* handler follows) */if(typeof console!=="undefined")console.warn("[tags] COMPANION_XP: no companion sheet named '"+cXpm[1].trim()+"' — award dropped (#136③)");R.muts.push("⚠ [COMPANION_XP:] "+cXpm[1].trim()+" — no companion by that name (dropped)");continue;}
...
function applyMutsTable(text,opts){
  ...
  /* audit A9: a CHAINED tag tail ("[NAME:a|b]|c|d]") is parsed only by WARES (#6 F11); every other family lost the tail
     while cleanTxt's warn claimed "the parser filed it". The display strip stays; the mutation log now records what was
     ignored, so the loss is attributable on the player's channel too. */
  var _tails=text.match(/\[([A-Z][A-Z_]+):[^\]]*\]\|[^\[\]\n|]{1,120}\|[^\[\]\n]{0,240}\]/g)||[],_ti;
  for(_ti=0;_ti<_tails.length;_ti++){var _tn=(_tails[_ti].match(/^\[([A-Z][A-Z_]+):/)||[])[1];if(_tn==="WARES")continue;var _tt=_tails[_ti].slice(_tails[_ti].indexOf("]")+1);
    R.muts.push("⚠ chained ["+_tn+":] tail ignored — one body per tag: "+_tt.slice(0,60));if(typeof console!=="undefined")console.warn("[tags] chained "+_tn+" tail NOT parsed (only [WARES:] chains are filed): "+_tt.slice(0,80));}
