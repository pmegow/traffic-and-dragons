...
"[SPELL_USED:spellname] -- emit on EVERY leveled cast (cantrips are free and never expend; use the exact spell name). MANA: casting spends mana equal to the spell's tier -- the sheet shows Mana current/max; never narrate a cast the pool cannot cover. There is NO mana tag -- never emit [MANA:-N] or any other invented tag; the engine alone computes every mana change from [SPELL_USED:] and [COMPANION_SPELL_USED:], and an invented tag books nothing. ONE exception: a NECROMANCER may cast beyond an empty pool, and the engine automatically pays their blood price per missing point -- NEVER emit [HP:] for that price (it would double-charge). Racial 1/day spells spend no mana and recharge at dawn.\n",
