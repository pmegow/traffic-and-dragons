...
function csSheetSections(c,invOwner,portable){
  ...
  var charKv=akaHtml+(c.appear?csKv("Appearance",escHtml(c.appear)):"")+(c.mark?csKv("Distinguishing Mark",escHtml(c.mark)):"")+(c.trait?csKv("Trait",escHtml(c.trait)):"")+(c.flaw?csKv("Flaw",escHtml(c.flaw)):"")+(c.motivation?csKv("Motivation",escHtml(c.motivation)):((typeof motivationSettledLine==="function"&&motivationSettledLine(c))?csKv("Motivation",escHtml("None standing — "+motivationSettledLine(c))):""))+(c.backstory?csKv("Backstory",escHtml(c.backstory)):"");/* user/model-authored prose (#22/UA18); #437 a settled purpose reads as closed */
...
function showCharSheet(){
  ...
  document.getElementById("cs-librep-btn").addEventListener("click",function(){showLibraryReplaceModal(c,function(){_csReRender();});});/* #428: the whole-sheet pull, confirmed */
...
function showNpcSheet(name){
  ...
  var modal=modalShell("npc-modal",/* #14 */
    "<div style='display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;'>"+(isParty&&sheet?"<div style='display:flex;gap:6px;flex-wrap:wrap;align-items:center;'><button id='npc-export-btn' style='font-size:11px;font-family:var(--font);padding:4px 10px;border:1px solid var(--brd);border-radius:var(--r);background:var(--bg2);color:var(--t1);cursor:pointer;'>Export Character</button><button id='npc-libupd-btn' title='Pull this companion&#39;s identity fields (appearance, backstory, portrait…) from their character-library copy — progression is never touched' style='font-size:11px;font-family:var(--font);padding:4px 10px;border:1px solid var(--brd);border-radius:var(--r);background:"+(storageAdapter.isServerMode()?"var(--bg2)":"var(--bg3)")+";color:"+(storageAdapter.isServerMode()?"var(--t1)":"var(--t2)")+";cursor:pointer;'>&#8635; Update from library</button><button id='npc-librep-btn' title='Replace this companion&#39;s WHOLE sheet — level, gear, spells, memories, everything earned in this campaign — with their character-library copy (asks first)' style='font-size:11px;font-family:var(--font);padding:4px 10px;border:1px solid var(--brd);border-radius:var(--r);background:"+(storageAdapter.isServerMode()?"var(--bg2)":"var(--bg3)")+";color:"+(storageAdapter.isServerMode()?"var(--t1)":"var(--t2)")+";cursor:pointer;'>&#10515; Replace from library</button></div>":"<span></span>")+"<button id='npc-x' style='background:none;border:none;color:var(--t2);font-size:24px;cursor:pointer;padding:0 4px;line-height:1;'>&#215;</button></div>"
  ...
  /* #428: the whole-sheet pull — the adopter swaps wsNpc.charSheet, so the sheet is re-read by name, never from the stale closure */
  if(document.getElementById("npc-librep-btn")){document.getElementById("npc-librep-btn").addEventListener("click",function(){showLibraryReplaceModal(sheet,function(){showNpcSheet(name);});});}
  csWireToggles(modal);
