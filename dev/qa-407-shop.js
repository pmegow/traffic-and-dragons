...
(async()=>{const browser=await chromium.launch({channel:'chrome',headless:true});try{
 ...
 assert.equal(await page.evaluate(()=>document.querySelector('#shop-go').disabled),true,'Complete is locked with nothing marked');
 await page.click('#shop-modal .shop-row[data-sell="rope"]');await page.click('#shop-modal .shop-row[data-sell="rope"]');
 await page.click('#shop-modal .shop-row[data-sell="bone-handled knife"]');await page.click('#shop-modal .shop-row[data-buy="healing potion"]');
 const marked=await page.evaluate(()=>({sell:[...document.querySelectorAll('#shop-modal .sel-sell')].map(e=>e.textContent.trim()),buy:[...document.querySelectorAll('#shop-modal .sel-buy')].map(e=>e.textContent.trim()),total:document.querySelector('#shop-modal .shop-total').innerText,go:document.querySelector('#shop-go').disabled}));
 ...
 await page.click('#shop-modal .shop-qty[data-clear="rope"]');assert.match(await page.evaluate(()=>document.querySelector('#shop-modal .shop-total').innerText),/−48 gp/,'clicking the count clears the stack');
 await page.click('#shop-modal .shop-row[data-sell="rope"]');await page.click('#shop-modal .shop-row[data-sell="rope"]');
 ...
 await page.click('#shop-go');await page.waitForFunction(()=>!document.querySelector('#shop-modal'));
