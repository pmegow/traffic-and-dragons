...
rc |= sabotage.prove({
  ...
  cases: [
    ...
    { label: "same-turn dedupe removed — one arrival books twice",
      ...
      find: "  if(rec.turns.indexOf(turn)>=0)return false;      /* same-turn dedupe — the FIRST writer's source stands */\n",
      replace: "" },
    { label: "cap fold gutted — turns grow without bound, no aggregate (amendment ①)",
      ...
      replace: "  while(false){" },
    { label: "npcRecordPresence split guard removed — a split member's derived presence becomes false attendance (#194 re-pointed: the guard moved from mapNpcLocation into the ONE presence writer)",
      mustFail: "the #137 split guard covers it",
