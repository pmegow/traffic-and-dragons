...
rc|=sabotage.prove({
  ...
  cases:[
    { label:"#294B ①: the inline-description exemption dies — a self-described item is nagged about anyway",
      mustFail:"an inline \"—\" description exempts itself",
    ...
    { label:"#294B ①: the asked latch dies — every change of hands re-asks (the bedroll nag the skip-list used to hide)",
      ...
      replace:"" },
    { label:"#294B ①: the latch is stamped at ARMING instead of delivery — a dead provider turn leaves the item asked-but-never-delivered",
      mustFail:"asked ONCE per item per campaign",
      find:"  delete worldState.itemDefCandidate;\n  _itemDefMarkAsked(cand.key,worldState.turn||0);",
      replace:"  delete worldState.itemDefCandidate;\n  if(!worldState.itemDefAsked||worldState.itemDefAsked[cand.key]==null)_itemDefMarkAsked(cand.key,cand.turn);worldState.itemDefAsked[cand.key]=cand.turn;" },
