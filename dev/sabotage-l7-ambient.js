...
rc |= sabotage.prove({also:also,file:"ambient.js",command:["node",["dev/run-tests.js","L7 ambient scene policy"]],cases:[
 ...
 {label:"closed or missing hours can start a fire",mustFail:"L7 fire needs enabled",find:"if (s.open !== true) return silent;",replace:""},
...
rc |= sabotage.prove({also:also,file:"ambient.js",command:["node",["dev/tests-l7-ambient.js"]],cases:[
 {label:"same-scene source is recreated on every UI sync",mustFail:"FAIL L7 same scene reuses one source",find:"if (source) { driver.gain(source, desired.gain); return; }",replace:"if (source) { driver.gain(source, desired.gain); }"},
 {label:"stale decoded audio is admitted after leaving",mustFail:"FAIL L7 leave during load",find:"!disposed && !job.cancelled && job.epoch === epoch) buffer = decoded",replace:"!disposed) buffer = decoded"},
