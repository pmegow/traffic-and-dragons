...
function runEngineTests(R){
  ...
  t("[SPELL_DEF:] files a GM-invented spell into the overlay; lookup returns it",function(){makeWorld();applyMuts("[SPELL_DEF:Frost Lance|range=60ft|targets=1 creature|duration=instantaneous|effect=a lance of ice; DEX save or 2d8 cold and slowed|cost=slot|tier=1|magical=yes]");var e=spellBibleLookup("Frost Lance");return e&&e.range==="60ft"&&e.cost==="slot"&&e.tier===1&&e.isMagical===true&&/lance of ice/.test(e.effect)?true:"bad overlay entry: "+JSON.stringify(e);});
  t("[SPELL_DEF:] is write-once — a second definition does not overwrite",function(){makeWorld();applyMuts("[SPELL_DEF:Frost Lance|range=60ft|effect=original]");applyMuts("[SPELL_DEF:Frost Lance|range=1 mile|effect=drifted]");var e=spellBibleLookup("Frost Lance");return e&&e.range==="60ft"?true:"write-once failed, got range "+(e&&e.range);});
  t("[SPELL_DEF:] magical=no sets isMagical false",function(){makeWorld();applyMuts("[SPELL_DEF:Mundane Trick|effect=sleight of hand|magical=no]");var e=spellBibleLookup("Mundane Trick");return e&&e.isMagical===false?true:"isMagical not false: "+JSON.stringify(e);});
  ...
  t("ability_bible: martial ability resolves and is isMagical:false",function(){var e=abilityBibleLookup("Power Strike");return e&&e.kind==="ability"&&e.isMagical===false?true:"bad: "+JSON.stringify(e);});
  t("ability_bible: arcane ability is isMagical:true",function(){var e=abilityBibleLookup("Arcane Bolt");return e&&e.isMagical===true?true:"arcane bolt not magical";});
  t("every ability entry carries kind/cost/isMagical/effect",function(){var bad=[];for(var k in ABILITY_BIBLE){var e=ABILITY_BIBLE[k];if(e.kind!=="ability"||!e.cost||typeof e.isMagical!=="boolean"||!e.effect)bad.push(k);}return bad.length?"malformed: "+bad.join(", "):true;});
  t("capabilityLookup resolves an ability-that-is-a-spell via spell_bible (no dup canon)",function(){var e=capabilityLookup("Sacred Flame");return e&&e.kind==="spell"?true:"Sacred Flame should resolve as the spell entry, got "+JSON.stringify(e&&e.kind);});
