...
(async()=>{
 for(const id of ['inworld','speechify']){
  await test('#401 '+id+' uses documented PCM request and direction controls',async()=>{const d=draft(id),c=d.models[id];c.direction='Whisper gently';c.rate=1.1;c.emotion='calm';c.language=id==='inworld'?'ko-KR':'en-US';let request;global.fetch=async(url,opts)=>{request={url,opts,body:JSON.parse(opts.body)};return id==='inworld'?{ok:true,json:async()=>({audioContent:Buffer.from([0,0,255,127]).toString('base64')})}:pcm()};let r=await S._fetch(id,{text:'문을 열어. <hello> & friends',voice:'a'},c,'fixture');assert.equal(r.rate,24000);assert.equal(r.bytes.length,4);assert.equal(request.opts.headers.Authorization,(id==='inworld'?'Basic':'Bearer')+' fixture');if(id==='inworld'){assert.equal(request.body.modelId,'inworld-tts-2');assert.equal(request.body.instruction,'Whisper gently');assert.equal(request.body.language,'ko-KR');assert.equal(request.body.audioConfig.audioEncoding,'PCM');assert.equal(request.body.audioConfig.speakingRate,1.1)}else{assert.equal(request.body.model,'simba-3.2');assert.equal(request.body.output_format,'pcm_24000');assert(request.body.input.includes('&lt;hello&gt; &amp; friends'));assert(!request.body.input.includes('speechify:style'));/* #454: Simba 3 ignores the style tag — a stale saved emotion is never sent */assert(request.body.input.includes('rate="+10%"'));assert(!request.body.instructions)}});
  for(const phase of ['headers','body'])await test('#401 '+id+' bounds stalled '+phase+' and cancellation',async()=>{let signal;global.setTimeout=(fn,ms)=>realTimer(fn,Math.min(ms,15));global.fetch=(u,o)=>{signal=o.signal;return phase==='headers'?new Promise(()=>{}):Promise.resolve({ok:true,json:()=>new Promise(()=>{}),arrayBuffer:()=>new Promise(()=>{})})};const r=await settled(S._fetch(id,{text:'wait',voice:'a'},draft(id).models[id],'fixture'));assert.match(r.fail,/timeout/);assert(signal.aborted)});
  await test('#401 '+id+' refuses malformed audio and paid errors without retries',async()=>{for(const status of [401,402,429,500]){let n=0;global.fetch=async()=>{n++;return{ok:false,status}};const r=await S._fetch(id,{text:'wait',voice:'a'},draft(id).models[id],'fixture');assert.match(r.fail,new RegExp(String(status)));assert.equal(n,1)}global.fetch=async()=>({ok:true,json:async()=>({audioContent:'AQID'}),arrayBuffer:async()=>new Uint8Array([1,2,3]).buffer});assert.match((await S._fetch(id,{text:'wait',voice:'a'},draft(id).models[id],'fixture')).fail,/Invalid/)});
  await test('#401 '+id+' audition uses draft without saving and close cancels request',async()=>{const d=draft(id),before=JSON.stringify(S.draft());let request;global.fetch=(u,o)=>{request=o;return new Promise(()=>{})};S.test(d,'A quiet road.','b',()=>{});await sleep(10);assert.equal(JSON.stringify(S.draft()),before);assert(request);TTS.stop();assert(request.signal.aborted);await sleep(5);assert.equal(sources.length,0)});
  await test('#401 '+id+' failed audition never plays another engine or changes selected provider',async()=>{let requested=[];const d=draft(id),before=S.draft().primary;global.fetch=async(u,o)=>{requested.push(u);return{ok:false,status:401}};const phases=[];S.test(d,'A quiet road.','b',p=>phases.push(p));await sleep(20);assert.equal(requested.length,1);assert.equal(S.draft().primary,before);assert.equal(phases.at(-1),'idle');assert.equal(TTS.isPlaying(),false);assert.equal(sources.length,0)});
 }
 ...
 await test('#401 malformed saved root does not crash the settings dialog',async()=>{const k=S._keys.settings,old=store.get(k);try{store.set(k,'null');assert(S.draft().models.inworld);store.set(k,'[]');assert(S.draft().models.inworld)}finally{if(old===null)store.del(k);else store.set(k,old)}});
 ...
 await test('#401 malformed or stalled 429 body remains bounded and attributable',async()=>{
  global.fetch=async()=>({ok:false,status:429,json:async()=>{throw Error('bad JSON')}});assert.match((await S._fetch('speechify',{text:'Wait.',voice:'a'},draft('speechify').models.speechify,'fixture')).fail,/HTTP 429/);
  ...
  assert.match((await settled(S._fetch('speechify',{text:'Wait.',voice:'a'},draft('speechify').models.speechify,'fixture'))).fail,/timeout/);assert(signal.aborted);
 });
