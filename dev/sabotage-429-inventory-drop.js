...
prove("helpers.js", ENGINE, [
  ...
  { label: "the button text loses its plural",
    find: 'return "Delete "+n+" item"+(n===1?"":"s");', replace: 'return "Delete "+n+" item";',
    mustFail: "the button text" },
  { label: "the button says Drop again (implies the items are on the floor — they are not)",
    find: 'return "Delete "+n+" item"+(n===1?"":"s");', replace: 'return "Drop "+n+" item"+(n===1?"":"s");',
    mustFail: "the button text" }
]);
prove("ui-sheets.js", SHEET, [
  ...
  { label: "a commit that resolves nothing is silent",
    find: 'if(!plan.ok){if(typeof showToast==="function")showToast("Nothing to delete"+stale,5000);_invSheetRepaint(owner);return;}', replace: 'if(!plan.ok){_invSheetRepaint(owner);return;}',
  ...
  { label: "the × on a marked row no longer reads as the un-mark",
    find: 'title="\'+(_marked?\'Keep this item (un-mark)\':\'Mark this item to delete\')+\'"', replace: 'title="Mark this item to delete"',
    mustFail: "marked rows read red" },
  ...
  { label: "the discard is silent",
    find: 'if(typeof showToast==="function")showToast(n+" mark"+(n===1?"":"s")+" cleared — nothing deleted",4000);', replace: 'void 0;',
