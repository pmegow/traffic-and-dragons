...
rc |= sabotage.prove({
  ...
  cases: [
    { label: "\\b anchoring dropped — 'Morningstar'/'knight' match again",
      find: "out.push(new RegExp(\"\\\\b(?:\"+TIME_PHASES[i].re.source+\")\\\\b\",\"gi\"));",
      replace: "out.push(new RegExp(\"(?:\"+TIME_PHASES[i].re.source+\")\",\"gi\"));" },
    { label: "any-quote-in-sentence rule removed — a quoted phase with no speech verb leaks",
      find: "    if(/[\"“”]/.test(sent))continue;",
      replace: "" },
    { label: "rejection list gutted — plans/history/figurative all assert",
      find: "if(_PHASE_REJECT_RE.test(sent))continue;",
      replace: "" },
    { label: "quote-parity guard removed — a stray quote no longer distrusts the entry",
      find: "  var _sq=(s.match(/\"/g)||[]).length;\n  if(_sq%2)return null;",
      replace: "" },
    { label: "last-cue selection becomes first-cue — narrative recency lost",
    ...
    { label: "band distance degrades to always-far — in-band agreement stops silencing (the DETECTOR's copy, not the reconcile's)",
    ...
    { label: "this/that reference guard removed — 'this afternoon' asserts",
...
rc |= sabotage.prove({
  ...
  command: ["node", ["dev/run-tests.js"]],
  cases: [
    { label: "stale-agreement discard removed — a healed clock still gets nagged",
