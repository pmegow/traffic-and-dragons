...
function createAudioCache(options){
  ...
  function serve(request){
    var path=new URL(request.url).pathname,asset=options.catalog.assets.filter(function(a){return "/"+a.bed.url===path;})[0];
    ...
    return options.caches.open(options.name).catch(function(e){warn(e);return null;}).then(function(cache){
      ...
      return cache.match(request).then(function(hit){
        ...
        return options.fetch(request).then(function(response){
          return audioReadBytes(response,Math.min(asset.bed.maxBytes,8*1024*1024)).then(function(bytes){return audioVerifyBytes(bytes,asset);}).then(function(bytes){
