...
function buildSkeletonReviewPrompt(skel,charCtx){
  ...
  return "Review this three-act campaign skeleton for a tabletop RPG run by an AI Game Master. Every structural gap you find is a gap the GM will fill by IMPROVISING mid-campaign — which causes canon drift over a long game. This is a ONE-PASS review whose fixes are applied automatically, unseen by a human: report only genuine, worth-fixing issues.\n\nSKELETON:\n"
    ...
    +"\n- REGISTER: a premise, act or arc built on a debt, ledger, contract, tax, toll, tithe, creditor or paperwork of any kind — this world keeps no books (debts here are blood, oaths, hunger and memory); a settled debt in THE RECORD is closed history and may not be reopened as the conflict"/* #437 */
    +(withDna?"\n- dnaHints that are generic procedure instead of arc-specific narrative direction":"")
    +(ctx?"\n- INVENTED PAST: when the CHARACTER carries a written backstory, THE RECORD or THE PLAYER'S OWN STAKE, any personal history the premise or arcs assert for the protagonist (a sibling, a lost lineage, an old failure, a debt) that they do not contain or that contradicts them, any premise that forgets a bond they name (a spouse, sworn family), and any premise that does not build on THE PLAYER'S OWN STAKE in the player's own terms. With none of the three, an invented past is allowed. ALWAYS: a premise that dates the protagonist's own past ('centuries ago', 'a lifetime before') — their years are the player's, not the skeleton's":"")
