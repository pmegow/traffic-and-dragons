...
The designer offers **Save to My Library** for personal account copies and **Publish to Catalog** only to server-confirmed admins. Publication selects an existing stable catalog id (preserving order) or appends a new campaign, with a spoiler-free description and an explicit byline preview. The server checks admin access, completeness, size, and revision conflicts; failures remain visible in the dialog. Both the home shelf and game browser use `storageAdapter.listBlueprintCatalog` with no-store reads. Reopening the game browser or returning to the home tab refreshes publication data. Blank author means no byline; no static fallback can revive an old author. Public data is separate from all personal-library copies.

The designer Library window defaults to My Library. A fresh server account check adds Catalog for admins only. Catalog reads fetch current published entries and Open retains the stable catalog id, so edits and renames publish back to the same campaign. The source registry shares the list renderer; only personal entries offer deletion. Opening either source asks before discarding a dirty draft. Read responses are scoped to both the current source request and modal instance; errors expose a Retry button.
