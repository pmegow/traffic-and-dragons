...
**audit 2026-09-18 — the memory lane (audits/audit_fable_2026_09_18.html §C).** ONE node factory, `newMapNode(firstVisit,parent,extra)` (memory.js), mints every map node; the optional fields a node carries are registered ONCE in `NODE_CARRY_FIELDS` with a fold policy (canon-wins / union-by-item / concat / newer / or) that `locFoldNodeRecords` applies on a merge and `locSplit` copies to the primary successor (owner and exits included) — a merge no longer drops a floor plan, market, hours, wants or rumour mill, and a split no longer drops the house's owner. `memory.locations[k].visited` is bounded to `VISITED_CAP` (50); `fileLocation` stamps `lastVisit` like `fileSubLocation`. `currentNodeKey` composes under the CANONICAL world (`locResolve` first), as do the place and audio compositions in memory.js. The extractor's `sameNpc` hint drops a provisional identity (the scan's #156 guard). `resolveFutureEvent` archives every removal (`resolvedBy: exact|substring`, the needle, the turn) and warns on a substring match. `fileWanted` warns on eviction and the WANTED handler logs the dropped want. An NPC merge carries `lastSeenAt/lastSeenTurn/lastSeenSrc/lastMentioned` (newer wins) and the memory-side pronouns/attitude. The geo block keeps a sub-location that HOLDS canon (layout, present items, open doors, wares, owner, state notes) listed past the 20-turn window. `sceneRefsSummaryFailure(hard)` warns on the third failure; `futureResolveOverlap` delegates to `feNearDup`; clock.js archives through `memArchive()`. Archives stay UNCAPPED (owner ruling 2026-09-16).
