...
| `csWireToggles` | 1075 | |
| `_csReRender` | — | audit E7: THE sheet re-render for every sheet action — in place (`refreshCharSheetInPlace`, #382b) when a sheet is open, a plain open otherwise. Only it and `refreshCharSheetInPlace` may call `showCharSheet()` to re-render |
