...
| Item | Finding |
|---|---|
| **Location** | `C:\Users\hannu\OneDrive\Documents\Projects\traffic-and-dragons-server\` — a **sibling** of the game repo, not inside it. (CLAUDE.md §22's `cd traffic-and-dragons-server && flyctl deploy` reads as if it were a subdirectory — it isn't.) |
...
3. **Move the dev `.env` and `ashen.db*` out of the OneDrive-synced tree** (or accept the sync
   deliberately and note it — but the OAuth secret should not remain there post-rotation).
