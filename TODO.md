...
| 3 | Portrait drag — reframe portrait by dragging | S | Ready to test — code verified (mouse + touch handlers wired, offset persists); needs phone check after deploy |
| 4 | iOS notch — topbar clipped under the notch, File button hard to tap | S | Ready to test — ROOT CAUSE: the mobile `@media (max-width:768px)` `#topbar { padding: 6px 12px }` shorthand was clobbering the safe-area `padding-top`, so every earlier bump was dead on mobile. Fixed by adding explicit `padding-top: max(6px, calc(env(safe-area-inset-top,0px) + 12px))` to the mobile rule. Needs phone check after deploy |
