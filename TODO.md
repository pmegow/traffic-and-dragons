...
| 3 | iOS notch — topbar clipped under the notch, File button hard to tap | S | ✅ Resolved (v1.33). ROOT CAUSE was the mobile `@media` `#topbar { padding: 6px 12px }` shorthand clobbering the safe-area `padding-top` — so the 13→18→23 bumps were all dead on mobile. After fixing that, the leftover `+12` additive sat ON TOP of the inset → too far down. Terminal fix: both `#topbar` rules now use `padding-top: max(Npx, env(safe-area-inset-top, 0px))` with NO additive — content sits exactly at the iOS safe boundary, the highest it can go without occlusion. Don't add px here again. |
