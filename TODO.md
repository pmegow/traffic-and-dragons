...
| 16 | Create story compiler, to take the narative sumaries and weave them together into a short story.  PDF final project, or html as a fallback | L | Planning doc written ([STORY_COMPILER.md](STORY_COMPILER.md)) — chunked compile, standalone HTML first, ~5.5h build estimate. Ready to build. |
...
| 2 | Portrait pan + zoom — reframe portrait by dragging, plus zoom (wheel / pinch / +− buttons) | S | ✅ Reworked (v1.39). Vertical pan was impossible: landscape (4:3) images in a portrait oval only overflow horizontally under object-fit:cover. Switched to transform-based pan+zoom — `portraitOffset={x,y,zoom}`, `applyPortraitTransform()`; zooming unlocks vertical slack so both axes pan. Drag + wheel + pinch + modal +/− buttons. Portraits now generate at 3:4. Legacy offsets auto-upconvert. **Pinch feel needs a phone check after deploy.** |
