...
| 13 | **[subtask]** Gemini (Google) — `generativelanguage.googleapis.com`; different body schema (`contents`, `generationConfig`), different response path; moderate adapter work | M | Pending. Pattern proven by #12: add a `PROVIDERS.gemini` object (headers/buildBody/parseResponse for the `contents` schema) + likely its own `reinforce` block — expect the same "narrates without emitting tags" gotcha. Verify against a money turn the same way. |
| 14 | **[subtask]** Grok (xAI) — OpenAI-compatible endpoint (`api.x.ai/v1`); adapter nearly identical to ChatGPT subtask; add after #12 | S | Pending. Should be nearly copy-paste of `PROVIDERS.openai` (OpenAI-compatible body/response). Reuse `openai.reinforce` as a starting point; live-verify tag fidelity on a money turn. |
| 15 | **[subtask / exploration]** Local LLM (Ollama) — OpenAI-compatible localhost API; near-zero adapter work after #12; CORS friction on https origin; tag system reliability is the real risk — 70B+ needed, rules out most consumer hardware including 16GB VRAM; revisit when quantized 70B quality improves | M | Exploration only. #12 confirmed the real risk is tag fidelity, not the adapter — a local model will need an even heavier `reinforce` and may still miss; the extraction-pass backstop (a second summarize-style call to pull tags from prose) is the fallback if reinforcement isn't enough. |
...
| 2 | Portrait drag — reframe portrait by dragging | S | Ready to test — code verified (mouse + touch handlers wired, offset persists); needs phone check after deploy |
| 4 | Test quest system (v1.34) — full live play-test with a real GM: GM offers a quest (⚑ toast + Opportunities), accept via journal AND via in-story agreement, confirm offered quests are NOT steered until accepted, objectives advance via `[QUEST_STEP:]`, completion archives + awards reward, decline archives. Watch for GM drift / hallucinated quest state across a summarization cycle. Also verify the GPT provider emits quest tags (reinforce covers it). | S | Ready to test |
| 3 | iOS notch — topbar clipped under the notch, File button hard to tap | S | ✅ Resolved (v1.33). ROOT CAUSE was the mobile `@media` `#topbar { padding: 6px 12px }` shorthand clobbering the safe-area `padding-top` — so the 13→18→23 bumps were all dead on mobile. After fixing that, the leftover `+12` additive sat ON TOP of the inset → too far down. Both `#topbar` rules use `padding-top: max(Npx, calc(env(safe-area-inset-top, 0px) / 2))` (v1.36) — the full inset (env alone) sat too low because topbar content lives in the corners (clear of the clock/battery), so it can ride into the safe zone; halving the inset is the chosen visual, still scales per-device. No CSS double-count exists (verified — #topbar is the only top inset). Tune the divisor, not a px additive. |
