...
| 3 | Legacy characters — saved characters appear as NPCs in other campaigns. Configurable % chance **per new NPC**. Drawn from the **Character Library** (server-side pool), excluding the current PC, already-met legacy chars, and names already present as NPCs. Once introduced, flagged (`legacyCharsUsed`) so they can't reappear in that campaign. Injected into the system prompt (`legacyBlock` in buildSysPrompt) for organic GM introduction over 1–2 turns. | — | ✅ Done (v1.54) — `checkLegacyCharacter()` rolls per-new-NPC in `applyMuts` (v1.53 fixed the load-only bug); draws from cached `legacyLibCache` via `loadLegacyLibrary()`→`storageAdapter.listCharLibrary`. **Requires a server connection** (library is server-side; the old offline campaign-scan was replaced v1.54). To test: connected + ≥1 library character that isn't the current PC + chance>0 + play until the GM introduces a new NPC (toast: "☠ A familiar face approaches…"). Console warns if enabled+rolled but no eligible library char. |
...
| 25 | rethink the suggested action buttons.  Buttons should just add their text to the input field, this way they can be player edited.  Right now, it seems like we're parsing a string to create the buttons, this seems... fragile.  We should decide there are three buttons, then assign text to them individually. | — |  |
| 26 | Cheers should stay open until acknowledged. | — |  |
| 27 | We neede to assign some sounds to certain actions.  For example a small chime for a cheers notification would be nice.  This will entail the design and implementation of a sound library. | — |  |
| 28 | we neeed to figure out a way to avoid repetive quests / game play.  Documents, Documents, Documents, Documents, turns this game into a clerk simulator... not fun.  Or... "bind these 12 souls one after the next" again.... not fun | — |  |
| 29 | the alchol skill doesn't seem to be working. | — |  |
| 30 | spells and inventory should have tool tips that describe what they are.  For example Silence, without a descriptor, it's impossible to know if that's an area of effect spell, to make an area silent, or a spell that removes the ability to speak. | — |  |

| 26 | Portrait storage bloat / localStorage quota — portraits (base64, ~22–52KB each) dominate state size and are multiplied several ways: (a) **companion portraits stored 2×** — both `npc.portrait` and `npc.charSheet.portrait` hold the same image; (b) **`snapshotActiveCamp()` duplicates the entire state** (incl. all portraits) into `tnd_camp_<id>_*` on EVERY `saveAll`, so the active campaign is stored twice (~884KB doubled in the Ammut save). With multiple campaigns this multiplies → real mobile-quota risk (iOS Safari is strict). When quota is hit, `saveCore` catches it, toasts "storage full", and falls back to in-memory `_m` — so the save silently doesn't persist across reload. **Fixes:** dedupe companion portrait (single source); skip the redundant snapshot of the *active* campaign (only snapshot on switch-away — `tnd_core_v10` already is the active state); consider storing portraits in their own dedup'd localStorage keys so snapshots don't copy them. Measured 2026-06-15 from the t1392 save: worldState 269KB (~228KB of it portraits), memory 173KB. | M | Pending — diagnosed; NOT the cause of the v1.48 "won't stick" report (that was the mislabeled Save/Download button), but a latent quota hazard on large/multi-campaign mobile use. |
| 27 | Cloud portrait propagation gap — setting a portrait doesn't advance `worldState.turn`, and `load()` only restores `data.portrait`/`data.npcPortraits` inside `if(serverTurn>localTurn)`. So a portrait set on device A (no turn played) never propagates to device B via the turn-gated reconcile. Likely the "can't pull Frizwick's portrait from the cloud onto mobile" half of the v1.48 report. **Fix options:** bump a lightweight version/dirty counter on portrait changes and gate the portrait restore on that (not just turn), or always reconcile portraits regardless of turn. Ties into the broader sync-authority rework. | M | Pending — flagged this session; deeper sync change than the v1.48 UI fix. |
| 28 | Character description from portrait (reverse of the existing text→image) — when a portrait is uploaded, use a vision model to read the image and write/augment the character's appearance fields (`appear`, `mark`, hair/eyes/build, visible gear). The inverse of "Generate from Character Sheet": instead of sheet→image, do image→description. Lives in the Edit Portrait modal (an "↧ Describe from this image" action after Upload/Apply) and/or as a step in char creation when a player uploads a face. Writes to `character.appear`/`mark` (offer to overwrite vs. append). Needs a vision-capable model — Claude (anthropic) supports image input; fal.ai is generation-only, so this likely routes through `callGM` with an image block or a dedicated vision call. Consider companions too (describe an uploaded companion portrait into its `charSheet`). | M | Pending — design noted |
| 29 | **Duplicate NPC entries (alias drift)** — found in the t1392 audit: same person stored under short AND full name; "Aldara" exists 3× (`Aldara`/`Aldara Perdrath`/`Aldara of Perdrath`), plus Morwen, Frizwick, Aelith, Harlan, Hardin, Valdun, Verity, Groom. Both variants inject into the prompt with split knowledge. Root: no auto-consolidation; GM never emitted `[NPC_ALIAS:]`/`[NPC_MERGE:]`. **Remedy:** (a) one-time merge in the save (absorb short→full, combine knowledge/events/aliases, fix relationship references); (b) engine — prompt rule to use canonical full names + emit alias, and/or a guarded heuristic that auto-aliases a new name that's a first-name prefix of an existing canonical (must avoid merging two distinct "Aldara"s). Deferred from v1.50 batch as the riskiest (needs careful merge + testing). | M | Pending — audit 2026-06-16; HIGH value, do in test round |
| 30 | Dead NPCs in the live roster — 5 dead NPCs (Vane Tharwick, Vanya Perdrath, Corvel, Halvorn, Edric) remain in `worldState.npcs` and inject into the prompt NPC list every turn. **Remedy:** filter `status` matching `\bdead\b` out of the prompt roster (keep them in `memory.npcs`). Use a word-boundary test so "deadly"/"dreadful" don't false-match. Deferred from v1.50 (fuzzy detection wants a test pass). | S | Pending — audit 2026-06-16 |
| 31 | NPCs missing a `pronouns` field — 7 NPCs (incl. party members) have no pronouns; engine should fall back to `charSheet.gender`/"they" when injecting rather than omitting. Cosmetic; v1.47 stopped new pronoun-corruption but doesn't backfill. | S | Pending — audit 2026-06-16 |

> **Audit fixes shipped v1.50 (2026-06-16):** future-event runaway cap/dedupe/splice (#1), map-edge ordering (#3), quest-status normalize (#4), inventory `xN` stacking (#5). Cleaned save written to `..._t1392_clean.json` for import. Deferred: #29/#30/#31 above.
