...
## v1.932 — the fire is CC0

The v1 fire was cut from a Unity Asset Store purchase; the EULA allows it embedded in the game but not as a file in a public repository, and this repository is public. Replaced with Nox_Sound's `Ambiance_Firecamp_Medium_Loop_Mono.wav` from the free Essentials Series (README: "All these sounds are under CC0 license."), the closest of the pack's three fires to v1's crest and level. New scripted pipeline `dev/prepare-ambience-loop.py` (numpy + wheel ffmpeg): DC removal, one-second raised-cosine tail/head overlap, RMS matched to v1 at −32 dBFS under a −6 dBFS ceiling, libmp3lame 128 kbps mono with the Xing/LAME gapless header — the script refuses to deliver an MP3 without that header, because a header-less encode (lameenc, tried first) shifts the loop by ~23 ms of encoder delay. Receipt: `Audio/Prepared/smithy-fire-v2.json`; source and delivery SHA-256 in `sfx/README.md`.

Verification: Chrome decodes the delivery to exactly 9.0050625 s / 432,243 samples, loop [0, 9.005), peak 0.279 (`Audio/Prepared/browser-audio-receipt.json`, all 16 checks). Exterior rollout QA green with the shorter fire (`exterior-browser-receipt.json`); its voice hook admitted only ≥17 s buffers and was lowered to ≥8 s. Ambient suite 9/9, transitions 7/7, 11/11 sabotage clauses caught with byte-identical restoration, full suite ALL GREEN at 2,179 assertions. Test fixtures now read the bed's own `loopEnd`/`maxSeconds` instead of a hardcoded 18. Owner listening of v2 is open; `Audio/Prepared/smithy-fire-audition.html` plays v2, two CC0 alternates and v1 side by side at the shipped gain. The four exterior beds carry the same license defect → TODO #412.
