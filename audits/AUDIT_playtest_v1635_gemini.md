...
| Invented tags | ⚠ Two: `[SCENE_DEATH:x]` ×10 and `[LOCATION_SIZE:size\|n]` ×4. Both unknown to the registry → warned by `__tagUnknownScan`, display-stripped by cleanTxt's catch-all, **zero mutation**. Every `SCENE_DEATH` rode beside a legitimate `[NPC:name\|dead\|enemy]` in the same response, so death canon never depended on the invention. Repeat dead-writes across turns (19/21/22, 34/35/39) were absorbed by the durable `npc.dead` guard. |
...
3. **Watch (no action)**: Gemini's invented-tag habit (`SCENE_DEATH`, `LOCATION_SIZE`) — the unknown-tag guard held; revisit only if a future run shows an invention that *carries* state the real vocabulary then misses.
4. **From the coherence revision (candidate remedies, owner to rule):**
