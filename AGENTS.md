...
| `class_bible.js` | ✅ Active (#72 — **data only, NOT engine-wired**) | THE single definitive source for character progression once the C6 sequence lands (classDef() → store swap → `CLSS` deleted); until then the engine still reads the legacy tables and this file is loaded ONLY by the satellites + the test harness (dev/load-engine.js slots it after capability_bible.js, mirroring its eventual real position). Generated from the live engine tables ([history](DOC/CLAUDE_HISTORY.md#class_biblejs--bible_editorhtml--the-72-build-out)) — 9 classes, 27 archetypes, spec-R2 level grids (class 2/5/7/9/11/13/15/17 · archetype 3/6/10/14/18 + capstone 20), `CLASS_XP_LEVELS` 1–20 (L1–10 shipped verbatim, 11–20 draft), C2 spellTiers, C5 skillSeeds. 234 level slots, 36 seeded from CLASS_FEATURES with their real names, 198 fill-phase blanks. Machine-REGENERATED by bible_editor.html's exporter — canonical form `JSON.stringify(x,null,2)` between the `>>> CLASS BIBLE DATA` markers, byte-pinned by the BIBLE EDITOR CONTRACT (run-tests.js); content hand-edits in canonical form are legal, format drift fails the build. Coverage guard: every spell NAME in it must resolve via `capabilityLookup` (a new spell and its capability entry land in the SAME commit). ⚠ In sw.js's network-first regex ONLY until C6-② adds it to the app shell — remove it from the regex then |
...
- `ARCHETYPES` — 3 archetypes per class (27 total — 9 classes × 3)
...
- **anthropic** — `https://api.anthropic.com/v1/messages`; `x-api-key` + `anthropic-dangerous-direct-browser-access: true`; system as a top-level `system` field — **a two-block array for gameplay turns**: `[{stable + cache_control:ephemeral}, {volatile}]` (prompt caching, see §6), plain string for sysOverride calls; response at `content[0].text`. Default model `Codex-sonnet-4-6` — **verify this string is current before starting work each session.**
...
Shared `TAG_REINFORCE` constant (globals.js) is assigned to every non-Codex provider's `reinforce` (Codex needs none). Model names in each provider's `models[]` should be verified current; the modal's dropdown is fixed to that list. **All four non-Codex adapters are shape-verified but each still needs a live tag-fidelity test (a money turn) once a key is available — same process that surfaced the gpt-4o gotcha.**
...
- **Local folder rename pending** — `dnd_rpg` → `traffic-and-dragons` (do in Explorer before opening Codex; then update hardcoded paths in `.Codex/settings.local.json` and `.Codex/hooks/stop-check.js`)
